...
13. Before handoff, use `get_material_list`, then `create_delivery_bundle` for Java work or `export_build` with `bedrock_mcpack` for an iPhone/Bedrock handoff. Never export when build validation, the hash-bound contract, or its certificate is invalid. Java delivery must include the selected artifact, exact materials, origin/rotation instructions, contract, certificate, checksums, and compatibility status. A Bedrock mobile pack must expose its placement-to-command reduction and commands-per-tick limit. Treat `.litematic` compatibility as unverified until a real Litematica round trip is recorded.
...
15. Use `export_build` with `schem` for real Java Sponge Schematic v3 output and `bedrock_mcpack` for a contract-valid iPhone-importable Behavior Pack. Use `import_schematic` for bounded analysis/preview input. Never return or echo unbounded imported placement arrays in hosted mode.
