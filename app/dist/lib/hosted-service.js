...
const boundedNonnegativeInteger = (value, fallback, maximum) => {
    const parsed = Number.parseInt(value ?? "", 10);
...
export function loadHostedServiceConfig(env = process.env) {
    ...
    if (enabled) {
        if (!configuredPath || !databasePath || !isAbsolute(configuredPath))
            issues.push("BLOCKWRIGHT_DATABASE_PATH must be an absolute persistent SQLite file path.");
        if (configuredProjectRoot && !isAbsolute(configuredProjectRoot))
            issues.push("BLOCKWRIGHT_PROJECT_ROOT must be an absolute persistent directory when supplied.");
        if (!baseUrl)
            issues.push("BLOCKWRIGHT_PUBLIC_URL must be a valid HTTPS origin.");
        if (!sessionSecret || sessionSecret.length < 32)
            issues.push("BLOCKWRIGHT_SESSION_SECRET must contain at least 32 characters.");
        if (!reviewTokenPepper || reviewTokenPepper.length < 32)
            issues.push("BLOCKWRIGHT_REVIEW_TOKEN_PEPPER must contain at least 32 characters.");
        if (!supportEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(supportEmail))
            issues.push("BLOCKWRIGHT_SUPPORT_EMAIL must be a valid operator-owned address.");
        if (!operatorToken || operatorToken.length < 32)
            issues.push("BLOCKWRIGHT_OPERATOR_TOKEN must contain at least 32 characters.");
        if (registrationAccessKey && registrationAccessKey.length < 32)
            issues.push("BLOCKWRIGHT_REGISTRATION_ACCESS_KEY must contain at least 32 characters when supplied.");
        ...
        if (Boolean(stripeSecretKey) !== Boolean(stripePriceId))
            issues.push("STRIPE_SECRET_KEY and STRIPE_STUDIO_PRICE_ID must either both be present or both be absent.");
        ...
        if (trustProxyHopsValue !== undefined && !/^[0-4]$/.test(trustProxyHopsValue))
            issues.push("BLOCKWRIGHT_TRUST_PROXY_HOPS must be an integer from 0 through 4.");
...
function validatePassword(password) {
    ...
    if (!/[a-z]/i.test(password) || !/\d/.test(password))
        throw new Error("Password must contain both a letter and a number.");
}
...
export class HostedServiceStore {
    config;
    database;
    constructor(config) {
    ...
    reviewRecord(row) {
        if (!row)
            return undefined;
        const decisions = JSON.parse(String(row.decisions_json));
        if (!Array.isArray(decisions))
            throw new Error("Stored review decisions are invalid.");
        return {
            schemaVersion: 1,
            id: String(row.id),
    ...
    async insert(record) {
      ...
      `).get(record.tenantId, now);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(record.id, record.tenantId, record.projectId, record.versionId, record.buildHash, record.tokenHash, record.access, record.createdBy, record.createdAt, record.expiresAt, record.revokedAt ?? null, JSON.stringify(record.decisions));
    ...
    async getById(id) {
        return this.reviewRecord(this.database.prepare("SELECT * FROM review_links WHERE id = ?").get(id));
    }
    async getByTokenHash(hash) {
        return this.reviewRecord(this.database.prepare("SELECT * FROM review_links WHERE token_hash = ?").get(hash));
    }
    async update(record, expected = record) {
        const identity = [record.id, record.tenantId, record.projectId, record.versionId, record.buildHash, record.tokenHash];
        const now = new Date().toISOString();
        ...
        if (changed !== 1)
            throw new Error("Review link changed, expired, was revoked, or no longer exists.");
    }
    async deleteProjectData(tenantId, projectId) {
    ...
    async register(input) {
        ...
        if (!tenantName || tenantName.length > 80)
            throw new Error("Workspace name must contain between 1 and 80 characters.");
        ...
        this.database.exec("BEGIN IMMEDIATE");
        try {
            const tenantCount = this.database.prepare("SELECT COUNT(*) AS count FROM tenants").get();
            if (tenantCount.count >= this.config.maxTenants)
                throw new Error("Account registration is temporarily unavailable.");
    ...
    `).get(email);
        ...
        if (!row || !safeCompare(actual, expected))
            throw new Error("Email or password is incorrect.");
    ...
    `).get(tokenHash(token, this.config.sessionSecret), now);
        if (!row)
            return undefined;
        return {
            userId: row.user_id,
    ...
    deleteAccount(principal) {
        ...
        const remaining = this.database.prepare("SELECT COUNT(*) AS count FROM memberships WHERE user_id = ?").get(principal.userId);
        if (remaining.count === 0)
            this.database.prepare("DELETE FROM users WHERE id = ?").run(principal.userId);
        return { deleted: true, tenantId: principal.tenantId };
    }
    billingState(tenantId) {
    ...
    requestRefund(principal, reason) {
        const cleanReason = reason.trim().replace(/\s+/g, " ");
        if (cleanReason.length < 10 || cleanReason.length > 1000)
            throw new Error("Refund reason must contain between 10 and 1000 characters.");
        ...
        return { id, status: "requested", createdAt, supportEmail: this.config.supportEmail };
    }
    recordUsage(principal, eventName, quantity = 1, estimatedCostMicrousd = 0) {
        ...
        if (!Number.isSafeInteger(quantity) || quantity < 0 || quantity > 10_000_000)
            throw new Error("Invalid telemetry quantity.");
        if (!Number.isSafeInteger(estimatedCostMicrousd) || estimatedCostMicrousd < 0 || estimatedCostMicrousd > 1_000_000_000)
...
export function sessionTokenFromHeaders(authorization, cookieHeader) {
...
export function validateBase64Upload(base64, maximumBytes) {
    if (!/^[A-Za-z0-9+/]*={0,2}$/.test(base64) || base64.length % 4 !== 0)
        throw new Error("Upload is not valid base64 data.");
    const bytes = Math.floor(base64.length * 3 / 4) - (base64.endsWith("==") ? 2 : base64.endsWith("=") ? 1 : 0);
    if (bytes > maximumBytes)
        throw new Error(`Upload exceeds the configured ${maximumBytes}-byte compressed limit.`);
    return bytes;
}
//# sourceMappingURL=hosted-service.js.map