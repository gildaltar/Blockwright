...
import { diffBuildRecords } from "./projects.js";
function normalizeCuboid(region) {
    const values = [region.min.x, region.min.y, region.min.z, region.max.x, region.max.y, region.max.z];
    if (!values.every(Number.isSafeInteger))
        throw new Error("Selected-region bounds must be safe integers.");
    return {
        min: {
...
export function calculatePlacementBounds(placements) {
...
export function reviseSelectedRegion(base, requestedRegion, replacementPlacements, options = {}) {
    ...
    for (const placement of base.placements) {
        ...
        if (baseCoordinates.has(key))
            throw new Error(`Base build contains duplicate placement coordinate ${key}.`);
        baseCoordinates.add(key);
    }
    const replacements = cloneJson(replacementPlacements);
    const replacementCoordinates = new Set();
    for (const placement of replacements) {
        if (![placement.x, placement.y, placement.z].every(Number.isSafeInteger))
            throw new Error("Replacement placements must use safe integer coordinates.");
        if (!inside(placement, region))
            throw new Error(`Replacement at ${coordinateKey(placement)} lies outside the inclusive selected region.`);
        const key = coordinateKey(placement);
        if (replacementCoordinates.has(key))
            throw new Error(`Replacement placements contain duplicate coordinate ${key}.`);
    ...
    if (!placements.length)
        throw new Error("A selected-region revision cannot produce an empty build.");
    ...
    const warningIssues = contract.warnings.map((result) => ({
