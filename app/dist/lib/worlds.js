...
function stableWorldId(canonicalPath) {
...
function asNumber(value) {
    ...
    if (Array.isArray(value) && value.length === 2)
        return value[0] * 0x1_0000_0000 + (value[1] >>> 0);
    return undefined;
}
function gameMode(value) {
    return { 0: "Survival", 1: "Creative", 2: "Adventure", 3: "Spectator" }[Number(value)] ?? "Unknown";
}
function isWorldLocked(worldPath) {
...
function detectPlatform(worldPath) {
    const savesRoot = dirname(worldPath);
    const instanceRoot = dirname(savesRoot);
    const serverRoot = dirname(worldPath);
    if (existsSync(resolve(serverRoot, "plugins", "WorldEdit")) || existsSync(resolve(serverRoot, "paper.yml")))
        return "paper";
    if (existsSync(resolve(serverRoot, "spigot.yml")))
        return "spigot";
    const mods = resolve(instanceRoot, "mods");
    if (existsSync(mods)) {
        const names = readdirSync(mods).map((name) => name.toLowerCase());
        if (names.some((name) => name.includes("neoforge")))
            return "neoforge";
        if (names.some((name) => name.includes("fabric") || name.includes("worldedit-fabric")))
            return "fabric";
        if (names.some((name) => name.includes("forge")))
            return "forge";
    }
    return safeRealPath(savesRoot).toLowerCase() === safeRealPath(defaultSavesRoot()).toLowerCase() ? "vanilla" : "unknown";
}
function suggestedFolders(worldPath, platform) {
    const savesRoot = dirname(worldPath);
    const instanceRoot = dirname(savesRoot);
    const serverRoot = dirname(worldPath);
    if (platform === "paper" || platform === "spigot")
        return [resolve(serverRoot, "plugins", "WorldEdit", "schematics")];
    return [resolve(instanceRoot, "config", "worldedit", "schematics")];
}
export async function readWorldMetadata(worldPath) {
    const canonicalPath = safeRealPath(worldPath);
    const levelPath = resolve(canonicalPath, "level.dat");
    if (!existsSync(levelPath) || !lstatSync(levelPath).isFile())
        throw new Error(`No level.dat found in ${canonicalPath}`);
    const stats = statSync(levelPath);
    if (stats.size > 32 * 1024 * 1024)
        throw new Error(`level.dat is unexpectedly large: ${canonicalPath}`);
    const parsed = await parse(readFileSync(levelPath), "big");
    const root = simplify(parsed.parsed);
...
export async function discoverWorlds(authorizedRoot) {
    const roots = [...new Set([defaultSavesRoot(), ...(authorizedRoot ? [resolve(authorizedRoot)] : [])])];
    const worlds = [];
    const warnings = [];
    for (const root of roots) {
        if (!existsSync(root)) {
            if (authorizedRoot && resolve(authorizedRoot) === root)
                warnings.push(`Authorized saves folder does not exist: ${root}`);
            continue;
        }
        let entries = [];
        try {
            ...
            warnings.push(`Could not read saves folder: ${root}`);
            continue;
        }
        for (const name of entries.slice(0, 500)) {
            const candidate = resolve(root, name);
            try {
                if (lstatSync(candidate).isDirectory() && existsSync(resolve(candidate, "level.dat")))
                    worlds.push(await readWorldMetadata(candidate));
            }
            catch (error) {
...
function safeSchematicName(name) {
    const value = name.trim().replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/^[-.]+|[-.]+$/g, "");
    if (!value)
        throw new Error("A valid schematic name is required.");
...
export async function installWorldEditSchematic(input) {
    ...
    if (world.id !== input.worldId)
        throw new Error("The selected world identifier does not match its canonical path.");
    ...
    if (!recommended.includes(resolve(targetFolder).toLowerCase()))
        throw new Error("The target must exactly match a WorldEdit schematics folder suggested for the selected instance/server.");
    const filename = safeSchematicName(input.name);
    const target = resolve(targetFolder, filename);
    if (dirname(target).toLowerCase() !== resolve(targetFolder).toLowerCase())
        throw new Error("Ambiguous schematic target path.");
    ...
    const preview = {
        ...
        directWorldEditing: "unimplemented",
    };
    if (!input.confirmed)
        return { ...preview, status: "confirmation_required", verified: false, bytes: schematic.bytes.byteLength, sha256: createHash("sha256").update(schematic.bytes).digest("hex") };
    if (existsSync(target) && !input.overwrite)
        throw new Error(`A schematic already exists at ${target}; choose another name or explicitly allow overwrite.`);
    mkdirSync(targetFolder, { recursive: true });
    const temp = `${target}.${process.pid}.tmp`;
    writeFileSync(temp, schematic.bytes);
    renameSync(temp, target);
    const installed = readFileSync(target);
    const verified = await importSchematic(installed);
    return {
        ...preview, status: "installed", bytes: installed.byteLength, sha256: createHash("sha256").update(installed).digest("hex"),
