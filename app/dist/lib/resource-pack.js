...
function zipMetadataError(detail) {
    throw new Error(`Resource pack has invalid ZIP central-directory metadata: ${detail}.`);
}
function checkedZipRange(offset, length, boundary, label) {
...
function readZipUint64(view, offset, boundary, label) {
    checkedZipRange(offset, 8, boundary, label);
    const low = view.getUint32(offset, true);
    const high = view.getUint32(offset + 4, true);
    if (high > 0x1fffff)
        zipMetadataError(`${label} exceeds JavaScript's safe integer range`);
...
function findZipEndOfCentralDirectory(bytes, view) {
    if (bytes.byteLength < ZIP_END_OF_CENTRAL_DIRECTORY_BYTES)
        zipMetadataError("the end record is missing");
    ...
    for (let offset = bytes.byteLength - ZIP_END_OF_CENTRAL_DIRECTORY_BYTES; offset >= earliest; offset -= 1) {
        ...
        if (offset + ZIP_END_OF_CENTRAL_DIRECTORY_BYTES + commentLength === bytes.byteLength)
            return offset;
    }
 ...
 */
export function assertResourcePackZipDirectory(bytes, maximumEntries) {
    ...
    if (needsZip64) {
        ...
        if (zip64Disk !== 0 || diskCount !== 1)
            zipMetadataError("multi-volume archives are not supported");
        ...
        if (zip64RecordSize < 44)
            zipMetadataError("the ZIP64 end record is too short");
    ...
    const centralDirectoryEnd = checkedZipRange(centralDirectoryOffset, centralDirectorySize, centralDirectoryBoundary, "the central directory");
    ...
    while (cursor < centralDirectoryEnd) {
        ...
        if (signature === ZIP_CENTRAL_DIRECTORY_DIGITAL_SIGNATURE) {
            ...
            if (cursor !== centralDirectoryEnd)
                zipMetadataError("the central-directory digital signature is not last");
            continue;
        }
        zipMetadataError("an unexpected central-directory record was found");
    }
    if (countedEntries !== declaredEntries)
        zipMetadataError("the declared and actual entry counts differ");
}
...
function readEntryBounded(entry, maximumBytes, aggregate, label) {
...
function assertPngDimensions(bytes, reference, maximumDimension, maximumPixels) {
...
export async function loadResourcePack(file, placements, configuredLimits = {}) {
    const limits = {
    ...
    if (file.size > limits.maximumInputBytes)
        throw new Error(`Resource pack exceeds the configured ${limits.maximumInputBytes}-byte input limit.`);
    ...
    if (paths.length > limits.maximumEntries)
        throw new Error(`Resource pack exceeds the configured ${limits.maximumEntries.toLocaleString()}-entry limit.`);
    ...
    for (const entry of Object.values(archive.files)) {
        ...
        if (size === undefined)
            throw new Error("Resource pack contains an entry without trustworthy expanded-size metadata.");
    ...
    const expandedBudget = { used: 0, maximum: limits.maximumExpandedBytes };
    ...
    const readJson = async (path) => {
        ...
        if (declaredSize === undefined || declaredSize > limits.maximumJsonBytes)
            throw new Error(`Resource-pack JSON exceeds the configured ${limits.maximumJsonBytes}-byte limit: ${path}`);
        ...
        try {
            ...
            throw new Error(`Resource-pack JSON is not valid UTF-8: ${path}`);
        }
        try {
    ...
    const resolvingModels = new Set();
    const resolveModel = async (reference, depth = 0) => {
        if (modelCache.has(reference))
            return modelCache.get(reference);
        if (resolvingModels.has(reference))
            throw new Error(`Resource-pack model inheritance cycle detected at ${reference}.`);
        if (depth >= limits.maximumModelDepth)
            throw new Error(`Resource-pack model inheritance exceeds ${limits.maximumModelDepth} levels.`);
    ...
    const textureUrl = async (reference) => {
        ...
        const pending = (async () => {
            ...
            if (!entry)
                return undefined;
            const declaredSize = declaredExpandedBytes(entry);
            if (declaredSize === undefined || declaredSize > limits.maximumTextureBytes)
                throw new Error(`Resource-pack texture exceeds the configured ${limits.maximumTextureBytes}-byte limit: ${reference}`);
    ...
    try {
        ...
        if (requests.size > limits.maximumTextureRequests)
            throw new Error(`Resource pack preview exceeds the configured ${limits.maximumTextureRequests.toLocaleString()} unique block-state limit.`);
