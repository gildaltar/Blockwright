...
import { ManagedTaskError, TASK_STATES, createTaskDiagnostic, isTerminalTaskState, sanitizeDiagnosticText, validateTaskWork, } from "./task-contract.js";
const phaseRank = {
    queued: 0,
    preparing: 1,
    planning: 2,
...
function normalizeRestoredSnapshot(value) {
    if (!value || typeof value !== "object")
        throw new Error("Task journal contains a non-object task record.");
    if (typeof value.id !== "string" || !/^task_[A-Za-z0-9_-]{8,80}$/.test(value.id))
        throw new Error("Task journal contains an invalid task id.");
    if (typeof value.operation !== "string" || !value.operation.trim())
        throw new Error(`Task journal record ${value.id} has no operation.`);
    if (!TASK_STATES.includes(value.state))
        throw new Error(`Task journal record ${value.id} has an invalid state.`);
    if (value.resultReference !== undefined && !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,239}$/.test(value.resultReference))
        throw new Error(`Task journal record ${value.id} has an invalid result reference.`);
    if (!value.progress || !Number.isSafeInteger(value.progress.sequence) || value.progress.sequence < 0)
        throw new Error(`Task journal record ${value.id} has invalid progress.`);
    if (!value.timing || !Number.isFinite(Date.parse(value.timing.queuedAt)) || !Number.isFinite(Date.parse(value.timing.updatedAt)))
        throw new Error(`Task journal record ${value.id} has invalid timing.`);
    return cloneSnapshot(value);
}
export class FileTaskJournal {
    path;
    constructor(path) {
        if (!path.trim())
            throw new Error("Task journal path is required.");
        this.path = path;
    }
    async load() {
        let text;
        ...
        if (parsed.schemaVersion !== 1 || !Array.isArray(parsed.tasks))
            throw new Error("Task journal has an unsupported or invalid schema.");
        return parsed.tasks.map((task) => normalizeRestoredSnapshot(task));
    }
    async save(tasks) {
...
export class TaskManager {
    ...
    nextTaskId() {
        for (let attempt = 0; attempt < 8; attempt += 1) {
            ...
            if (!/^task_[A-Za-z0-9_-]{8,80}$/.test(candidate))
                throw new Error("Task id factories must return task_ followed by 8-80 URL-safe characters.");
            if (!this.tasks.has(candidate))
                return candidate;
        }
        throw new Error("Task id factory repeatedly returned an existing id.");
    }
    touch(snapshot, finished = false) {
        ...
        if (finished)
            snapshot.timing.finishedAt = snapshot.timing.updatedAt;
    }
    schedulePersistence() {
        if (!this.journal)
    ...
    mergeWork(previous, incoming, samePhase) {
        const normalized = validateTaskWork(incoming);
        if (!normalized || !previous || !samePhase || normalized.unit !== previous.unit)
            return normalized;
        const merged = { ...normalized };
        if (previous.completedUnits !== undefined)
            merged.completedUnits = Math.max(previous.completedUnits, normalized.completedUnits ?? 0);
        if (previous.totalUnits !== undefined)
            merged.totalUnits = Math.max(previous.totalUnits, normalized.totalUnits ?? 0);
        if (merged.completedUnits !== undefined && merged.totalUnits !== undefined)
            merged.totalUnits = Math.max(merged.totalUnits, merged.completedUnits);
        if (previous.cacheHits !== undefined)
            merged.cacheHits = Math.max(previous.cacheHits, normalized.cacheHits ?? 0);
        if (previous.cacheReusedUnits !== undefined)
            merged.cacheReusedUnits = Math.max(previous.cacheReusedUnits, normalized.cacheReusedUnits ?? 0);
        return merged;
    }
    report(record, update) {
        if (isTerminalTaskState(record.snapshot.state) || record.snapshot.state === "cancelling")
            return;
        ...
        if (!(currentComparable in phaseRank) || phaseRank[requestedPhase] < phaseRank[currentComparable])
            return;
    ...
    finish(record, state, diagnostic) {
        ...
        if (diagnostic)
            record.snapshot.diagnostic = diagnostic;
        else
            delete record.snapshot.diagnostic;
        if (state !== "completed")
    ...
    diagnostic(record, error, defaults = {
    ...
    async execute(record) {
        ...
        try {
            ...
            if (isTerminalTaskState(record.snapshot.state))
                return;
            if (record.controller.signal.aborted || record.snapshot.state === "cancelling") {
            ...
            if (record.resultReference) {
                ...
                if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,239}$/.test(reference))
            ...
            if (isTerminalTaskState(record.snapshot.state))
                return;
            if (record.controller.signal.aborted || record.snapshot.state === "cancelling" || (error instanceof ManagedTaskError && error.code === "TASK_CANCELLED")) {
                this.finish(record, "cancelled");
            }
            else {
    ...
    pump() {
        ...
        while (this.running < this.concurrency && this.queue.length) {
            ...
            if (!record || record.snapshot.state !== "queued" || !record.run)
                continue;
    ...
    async restore() {
        ...
        for (const raw of restored) {
            ...
            if (!isTerminalTaskState(snapshot.state)) {
                ...
                if (record.snapshot.timing.startedAt)
                    record.snapshot.timing.runMs = Math.max(0, now - dateValue(record.snapshot.timing.startedAt, now));
            }
        }
        this.enforceRetention();
    ...
    async submit(submission) {
        if (this.closed)
            throw new Error("Task manager is closed.");
        if (!submission.operation.trim())
        ...
        if (accepted >= this.concurrency + this.maximumQueued)
            throw new TaskQueueFullError(this.maximumQueued);
    ...
    getResult(taskId) {
        ...
        if (!record || record.snapshot.state !== "completed" || !record.snapshot.resultAvailable)
            return undefined;
    ...
    async cancel(taskId) {
        const record = this.tasks.get(taskId);
        if (!record || isTerminalTaskState(record.snapshot.state))
    ...
    async waitForTerminal(taskId, timeoutMs = 30_000) {
        ...
        return new Promise((resolve, reject) => {
            ...
            unsubscribe = this.subscribe((snapshot) => {
                if (snapshot.id !== taskId || !isTerminalTaskState(snapshot.state))
                    return;
