...
const cardinalSides = ["north", "south", "east", "west"];
const coordinateKey = ({ x, y, z }) => `${x},${y},${z}`;
const boundedCoordinates = (coordinates) => coordinates.slice(0, 250).map(({ x, y, z }) => ({ x, y, z }));
...
export function calculateBuildHash(input, placements) {
...
function featureSeverity(feature, explicit) {
    if (explicit)
        return { severity: explicit, text: feature.trim() };
    const match = feature.match(/^\s*(hard|warning|aesthetic)\s*:\s*(.+)$/i);
    return match
        ? { severity: match[1].toLowerCase(), text: match[2].trim() }
        : { severity: "hard", text: feature.trim() };
}
function clause(id, severity, requirement, source, sourceText, evaluator, parameters = {}) {
    return { id, severity, requirement, source, sourceText, evaluator, supported: Boolean(evaluator), parameters };
}
function requestedWidth(text, subject) {
...
function parseFeature(candidate, source, sourceIndex) {
    ...
    if (!text)
        return [clause(prefix, parsed.severity, "Non-empty requirement", source, input, undefined)];
    ...
    if (entranceWords.test(text)) {
        ...
        clauses.push(clause(`${prefix}-entrances`, "hard", `${sides.length ? sides.join(", ") : "declared"} entrance${sides.length === 1 ? "" : "s"} with at least ${requestedWidth(text, "entrance")} block(s) clear width`, source, input, "entrances", { sides: sides.length ? sides : ["south"], minimumWidth: requestedWidth(text, "entrance") }));
    }
    if (/\b(corridors?|paths?|passages?|clearance)\b/.test(text)) {
        clauses.push(clause(`${prefix}-clearance`, "hard", `${requestedWidth(text, "corridor")}-block-wide ${/\bcross\b/.test(text) ? "cross-corridor" : "entrance corridor"} with two-block headroom`, source, input, "corridor-clearance", { minimumWidth: requestedWidth(text, "corridor"), cross: /\bcross\b/.test(text), headroom: 2 }));
    }
    if (/\b(pedestal|spawn platform)\b/.test(text)) {
        ...
        clauses.push(clause(`${prefix}-spawn-pedestal`, "hard", `safe central ${size}x${size} spawn pedestal${material ? ` made from ${material}` : ""}`, source, input, "spawn-pedestal", { size, ...(material ? { material } : {}) }));
    }
    ...
    if (/\b(lit|lights?|lighting|illuminated|lanterns?|torches?)\b/.test(text)) {
        ...
        clauses.push(clause(`${prefix}-lighting`, "hard", spawnScoped ? "lighting at the central spawn" : "distributed interior lighting", source, input, "lighting", { minimumCount: Number(explicitCount?.[1] ?? 1), scope: spawnScoped ? "spawn" : "interior", distributed: /\b(well|throughout)\b/.test(text) }));
    }
    if (/\bcovered porch\b/.test(text))
        clauses.push(clause(`${prefix}-covered-porch`, "hard", "covered porch", source, input, "covered-porch"));
    if (/\b(hearth|fireplace)\b/.test(text))
        clauses.push(clause(`${prefix}-hearth`, "hard", "constructed hearth", source, input, "hearth"));
    if (/\bstorage loft\b|\bloft storage\b/.test(text))
        clauses.push(clause(`${prefix}-storage-loft`, "hard", "constructed storage loft", source, input, "storage-loft"));
    if (/\b(functional|furnished|usable) interiors?\b/.test(text))
        clauses.push(clause(`${prefix}-functional-interior`, "hard", "functional constructed interior", source, input, "functional-interior"));
    ...
    if (/\b(supported roofs?|support|structural contact)\b/.test(text))
        clauses.push(clause(`${prefix}-support`, "hard", "face-supported structural placements", source, input, "support-contact"));
...
export function normalizeBuildContract(input, override) {
    const clauses = implicitClauses.map((item) => ({ ...item, sourceText: item.requirement, parameters: { ...item.parameters } }));
    ...
    input.features.forEach((feature, index) => {
        ...
        if (mapped) {
            clauses.push(clause(`feature-${index + 1}-design`, "hard", mapped.text, "feature", feature, "design-assertions", { requirementId: mapped.id, elementIds: [...mapped.elementIds].sort(), assertionKinds: (mapped.assertions ?? []).map(({ kind }) => kind).sort() }));
            return;
        }
    ...
    input.design.requirements.forEach((requirement, index) => {
        if (mappedFeatureTexts.has(normalizeText(requirement.text)))
            return;
        clauses.push(clause(`design-${index + 1}-${requirement.id}`, "hard", requirement.text, "feature", requirement.text, "design-assertions", { requirementId: requirement.id, elementIds: [...requirement.elementIds].sort(), assertionKinds: (requirement.assertions ?? []).map(({ kind }) => kind).sort() }));
    });
    const extra = [
        ...(override?.features ?? []),
        ...(override?.clauses ?? []),
...
function expectedEnvelope(build) {
...
function entranceSide(placement, envelope) {
    // Minecraft convention: north is decreasing z; south is increasing z.
    if (placement.z === envelope.min.z)
        return "north";
    if (placement.z === envelope.max.z)
        return "south";
    if (placement.x === envelope.min.x)
        return "west";
    if (placement.x === envelope.max.x)
        return "east";
    return undefined;
}
function isLowerDoorHalf(placement) {
...
function detectEntrances(build) {
    ...
    for (const placement of build.placements) {
        if (!/_door$/.test(placement.block) || /_trapdoor$/.test(placement.block))
            continue;
        if (!isLowerDoorHalf(placement))
            continue;
        const side = entranceSide(placement, envelope);
        if (side)
            coordinates[side].push(placement);
    }
    const widths = {};
...
function isInsideEnvelope(point, envelope) {
...
function interiorLightingPlacements(build) {
...
function findSafeSpawn(build, byCoordinate) {
    const envelope = expectedEnvelope(build);
    ...
    for (let y = envelope.min.y; y <= envelope.max.y - 2; y += 1) {
        if (!byCoordinate.has(`${x},${y},${z}`))
            continue;
        if (!byCoordinate.has(`${x},${y + 1},${z}`) && !byCoordinate.has(`${x},${y + 2},${z}`))
            return { x, y: y + 1, z };
    }
    return undefined;
}
function supportFailures(build, byCoordinate) {
    const deltas = [
    ...
    const coordinates = [];
    for (const placement of build.placements) {
        if (/landscap|garden|foliage|path|terrain/i.test(placement.phase) || /grass|flower|leaves|vine|moss|carpet|snow/.test(placement.block))
            continue;
        if (deltas.some((delta) => byCoordinate.has(`${placement.x + delta.x},${placement.y + delta.y},${placement.z + delta.z}`)))
            continue;
        total += 1;
        if (coordinates.length < 250)
            coordinates.push({ x: placement.x, y: placement.y, z: placement.z });
    }
    return { total, coordinates };
}
function graphReachability(build) {
    const ids = build.plan.roomGraph.rooms.map(({ id }) => id);
    if (!ids.length)
        return { connected: false, reached: 0, total: 0 };
...
export function auditBuildSemantics(build) {
    const envelope = expectedEnvelope(build);
    const byCoordinate = new Map();
    for (const placement of build.placements)
        byCoordinate.set(coordinateKey(placement), placement);
    const entrances = detectEntrances(build);
    const entrancePlacements = cardinalSides.flatMap((side) => entrances.coordinates[side]);
    const blockedEntranceCoordinates = [];
    for (const placement of entrancePlacements) {
        ...
        for (let distance = 1; distance <= 2; distance += 1)
            for (let dy = 0; dy < 2; dy += 1) {
                ...
                if (byCoordinate.has(coordinateKey(point)))
                    blockedEntranceCoordinates.push(point);
            }
    }
    const spawn = findSafeSpawn(build, byCoordinate);
    const lights = interiorLightingPlacements(build);
    let functionalTotal = 0;
    const functionalCoordinates = [];
    for (const placement of build.placements) {
        if (!/interior|furnitur|hearth|fireplace|storage|workstation|seating|bed|loft/i.test(placement.phase))
            continue;
        functionalTotal += 1;
        if (functionalCoordinates.length < 250)
            functionalCoordinates.push({ x: placement.x, y: placement.y, z: placement.z });
    }
    const unsupported = supportFailures(build, byCoordinate);
    const palette = new Set([
        ...Object.values(build.input.rolePalette).filter((value) => typeof value === "string"),
        ...build.input.palette,
    ...
    const illegalPaletteCoordinates = [];
    for (const placement of build.placements) {
        if (palette.has(placement.block))
            continue;
        illegalPaletteTotal += 1;
        if (illegalPaletteCoordinates.length < 250)
            illegalPaletteCoordinates.push({ x: placement.x, y: placement.y, z: placement.z });
    }
...
function semanticCheck(audit, id) {
    return audit.checks.find((check) => check.id === id);
}
function hardResult(clause, status, message, expected, actual, coordinates) {
    return {
        clauseId: clause.id,
        severity: "hard",
        status,
...
function resultFromSemantic(clause, check) {
...
function centeredRange(minimum, maximum, width) {
...
function corridorResult(build, clause) {
    ...
    if (cross) {
        for (const x of centeredRange(envelope.min.x + 1, envelope.max.x - 1, width)) {
            for (let z = envelope.min.z + 1; z < envelope.max.z; z += 1)
                for (const y of yValues)
                    if (byCoordinate.has(`${x},${y},${z}`))
                        blocked.push({ x, y, z });
        }
        for (const z of centeredRange(envelope.min.z + 1, envelope.max.z - 1, width)) {
            for (let x = envelope.min.x + 1; x < envelope.max.x; x += 1)
                for (const y of yValues)
                    if (byCoordinate.has(`${x},${y},${z}`))
                        blocked.push({ x, y, z });
        }
        ...
        if (!entrances.length)
            return hardResult(clause, "unevaluated", "No boundary entrance exists from which to evaluate a corridor.", "a clear path from a boundary entrance", "no entrance");
        ...
        for (const entrance of entrances) {
            ...
            for (let step = 1; step <= distance; step += 1)
                for (const y of [entrance.y, entrance.y + 1]) {
    ...
    return hardResult(clause, blocked.length ? "fail" : "pass", blocked.length ? "Canonical blocks obstruct the requested corridor headroom." : "The requested corridor volume has two-block headroom.", `${width}-block-wide ${cross ? "cross" : "entrance"} corridor`, `${blocked.length} blocked cells`, blocked);
}
function pedestalResult(build, clause) {
    ...
    for (let y = envelope.min.y; y <= envelope.max.y - 2; y += 1) {
        ...
        if (platform.some((placement) => !placement))
            continue;
        if (material && platform.some((placement) => placement.block !== material))
            continue;
        if (!material && platform.some((placement) => !/spawn|pedestal/i.test(placement.phase)))
            continue;
        const blocked = xs.flatMap((x) => zs.flatMap((z) => [1, 2].map((dy) => ({ x, y: y + dy, z })))).filter((point) => byCoordinate.has(coordinateKey(point)));
        if (blocked.length)
            return hardResult(clause, "fail", "The central pedestal exists but lacks two-block headroom.", `${size}x${size} clear pedestal`, `${blocked.length} blocked headroom cells`, blocked);
        return hardResult(clause, "pass", "A centered pedestal with clear spawn headroom is present.", `${size}x${size}${material ? ` ${material}` : ""} pedestal`, `platform at y=${y}`, platform);
    }
...
function lightingResult(build, clause) {
    const lights = interiorLightingPlacements(build);
    ...
    return hardResult(clause, passes ? "pass" : "fail", passes ? "Explicit luminous placements satisfy the requested scope." : "The requested explicit lighting count or distribution is absent; no engine-light propagation was inferred.", `${minimum} explicit light(s)${scope === "spawn" ? " near spawn" : " in the interior"}${distributed ? ", distributed" : ""}`, `${relevant.length} matching light(s)${distributed ? ` across ${quadrants} quadrant(s)` : ""}`, relevant);
}
function assertionPlacementScope(requirement, assertion, matches) {
...
function materialTagsByBlock(build) {
    const tags = new Map();
    for (const material of Object.values(build.input.materialLibrary)) {
        if (typeof material === "string" || !material.tags?.length)
            continue;
        const blockTags = tags.get(material.block) ?? new Set();
        material.tags.forEach((tag) => blockTags.add(tag));
        tags.set(material.block, blockTags);
    }
    return tags;
}
function evaluateDesignAssertion(build, requirement, assertion, matches) {
    const scoped = assertionPlacementScope(requirement, assertion, matches);
    const scopedIds = new Set(assertion.elementIds?.length ? assertion.elementIds : requirement.elementIds);
    const generatedIds = new Set(scoped.map(({ elementId }) => elementId).filter((id) => Boolean(id)));
    ...
    switch (assertion.kind) {
        ...
        case "element_instances": {
            const instances = new Set(scoped.map(({ elementInstanceId }) => elementInstanceId).filter((id) => Boolean(id)));
        ...
        case "path_geometry": {
            const paths = build.input.design.elements.filter((element) => element.kind === "sweep" && scopedIds.has(element.id) && generatedIds.has(element.id));
        ...
        case "boundary_contact": {
            const envelope = expectedEnvelope(build);
            const counts = Object.fromEntries(assertion.sides.map((side) => [side, scoped.filter((placement) => (side === "west" ? placement.x === envelope.min.x
                        ...
                        : side === "north" ? placement.z === envelope.min.z
                            : side === "south" ? placement.z === envelope.max.z
                                : side === "bottom" ? placement.y === envelope.min.y
                                    : placement.y === envelope.max.y)).length]));
...
function designAssertionResult(build, clause) {
    ...
    if (!requirement)
        return hardResult(clause, "fail", "The normalized requirement is absent from the hash-bound design program.", requirementId, "missing");
    try {
    ...
    const matches = build.placements.filter((placement) => Boolean(placement.elementId && elementIds.has(placement.elementId) && placement.requirementIds?.includes(requirementId)));
    ...
    return hardResult(clause, pass ? "pass" : "fail", pass ? "Every source-grounded design assertion passed against canonical geometry." : "One or more source-grounded design assertions failed against canonical geometry.", [`${elementIds.size} mapped element(s)`, ...results.map(({ expected }) => expected)].join("; "), [`${generatedElementIds.size} generated element(s)${missing.length ? `; missing ${missing.join(", ")}` : ""}`, ...results.map(({ actual }, index) => `${requirement.assertions[index].kind}: ${actual}`)].join("; "), results.flatMap(({ coordinates }) => coordinates));
}
function evaluateHardClause(build, clause, audit) {
    ...
    switch (clause.evaluator) {
        ...
        case "entrances": {
            const sides = (clause.parameters.sides ?? ["south"]);
            const minimum = Number(clause.parameters.minimumWidth ?? 1);
            const missing = sides.filter((side) => (audit.entranceWidths[side] ?? 0) < minimum);
            return hardResult(clause, missing.length ? "fail" : "pass", missing.length ? `Boundary door geometry does not satisfy ${missing.join(", ")}.` : "Boundary door geometry satisfies every required side and width.", sides.map((side) => `${side}>=${minimum}`).join(", "), cardinalSides.map((side) => `${side}=${audit.entranceWidths[side] ?? 0}`).join(", "));
        }
...
function createCertificate(build, hardResults, warnings, aesthetics) {
    const hard = {
...
export function validateBuildContract(build, override, existingAudit) {
    ...
    const declaredWarnings = normalizedClauses
    ...
    const auditWarnings = audit.checks
    ...
    const registryWarnings = build.registry.note
