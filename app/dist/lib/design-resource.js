...
function safeAdd(...values) {
    ...
    for (const value of values) {
        if (!Number.isFinite(value) || value >= LIMIT - total)
            return LIMIT;
...
function safeMultiply(...values) {
    ...
    for (const value of values) {
        ...
        if (!Number.isFinite(normalized) || (normalized !== 0 && total > LIMIT / normalized))
            return LIMIT;
        total *= normalized;
    }
    return total;
}
function inclusiveSpan(a, b) {
...
function roundedPositive(value, fallback = 1) {
    return Math.max(1, Math.round(value ?? fallback));
}
function elementInstances(element) {
    return Math.max(1, element.offsets?.length ?? 0);
}
function sweepSampleUpperBound(points) {
    if (points.length < 2)
        return 0;
    return safeAdd(1, ...points.slice(0, -1).map((point, index) => {
        ...
        return Math.max(Math.abs(next.x - point.x) + Math.abs(next.y - point.y) + Math.abs(next.z - point.z), 1);
    }));
}
/**
 ...
 */
export function estimateDesignPlacementAttempts(program) {
    let total = 0;
...
//# sourceMappingURL=design-resource.js.map