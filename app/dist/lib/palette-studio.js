...
export function validatePaletteIdentifiers(edition, version, roles) {
    const registry = edition === "java" ? readJavaRegistry(version) : undefined;
    if (edition === "java" && !registry)
...
export function defaultRolePalette(style, edition, version, overrides = {}) {
    ...
    if (!overrideCheck.valid)
        throw new Error(`Invalid palette blocks: ${overrideCheck.invalid.map(({ role, block }) => `${role}=${block}`).join(", ")}`);
    ...
    if (!checked.valid) {
        for (const { role } of checked.invalid)
            proposed[role] = universalRoles[role];
    }
    const finalCheck = validatePaletteIdentifiers(edition, version, proposed);
    if (!finalCheck.valid)
        throw new Error(`Palette contains invalid ${edition} ${version} blocks: ${finalCheck.invalid.map(({ role, block }) => `${role}=${block}`).join(", ")}`);
    return finalCheck.normalized;
}
const interviewOrder = ["buildType", "architecturalDirection", "biome", "mood", "interpretation", "constraints", "dominantMaterials", "contrast", "weathering", "landscaping", "avoidBlocks", "resourcePack"];
...
export function continuePaletteInterview(input) {
    ...
    if (session.edition !== input.edition || session.version !== input.version)
        throw new Error("A palette interview cannot switch edition or version mid-session. Start a new session.");
    ...
    if (!changes.valid)
        throw new Error(`Invalid palette blocks: ${changes.invalid.map(({ role, block }) => `${role}=${block}`).join(", ")}`);
    for (const [role, block] of Object.entries(changes.normalized)) {
        if (!session.lockedRoles.includes(role))
            session.roles[role] = block;
    }
    ...
    if (rejectedAssignments.length)
        throw new Error(`Rejected blocks remain assigned: ${rejectedAssignments.map(([role, block]) => `${role}=${block}`).join(", ")}. Replace them before continuing.`);
...
export function savePalette(sessionId, name) {
    const db = readDatabase();
    const session = db.sessions.find(({ id }) => id === sessionId);
    if (!session)
        throw new Error(`Palette session not found: ${sessionId}`);
    const now = new Date().toISOString();
    const existing = db.palettes.find(({ id }) => id === sessionId.replace("palette_session_", "palette_"));
    const saved = { ...session, id: existing?.id ?? session.id.replace("palette_session_", "palette_"), name: name.trim() || session.name, createdAt: existing?.createdAt ?? now, updatedAt: now };
    delete saved.complete;
