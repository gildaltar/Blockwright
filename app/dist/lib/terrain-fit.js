...
const isAir = (block) => /(^|:)air$/.test(block);
const isWater = (block) => /(^|:)(water|flowing_water)$|kelp|seagrass/.test(block);
const isPath = (block) => /dirt_path|gravel|path|road/.test(block);
const isVegetation = (block) => /leaves|log|flower|grass|fern|sapling|vine|mushroom|bush/.test(block);
const isSensitive = (placement) => Boolean(placement.blockEntity)
...
function assertSafeInteger(value, label) {
    if (!Number.isSafeInteger(value))
        throw new Error(`TERRAIN_FIT_INVALID: ${label} must be an integer.`);
}
function normalizedInterface(input) {
    if (!TERRAIN_INTERFACE_STRATEGIES.includes(input.strategy))
        throw new Error(`TERRAIN_FIT_INVALID: unsupported strategy ${String(input.strategy)}.`);
...
function validateMatrix(matrix, width, depth, label) {
...
function createRegionGrid(region) {
    const { width, depth } = region.dimensions;
    if (width * depth > 1_048_576)
        throw new Error("TERRAIN_FIT_LIMIT: world-region analysis is limited to 1,048,576 columns.");
    ...
    for (const placement of region.blocks ?? []) {
        if (!insideBounds(placement, bounds) || isAir(placement.block))
            continue;
        const x = placement.x - region.origin.x;
        const z = placement.z - region.origin.z;
        const key = key2(x, z);
        const current = topByColumn.get(key);
        if (!current || current.y < placement.y)
            topByColumn.set(key, placement);
    }
    ...
    if (region.surfaceBlocks) {
        ...
        for (let z = 0; z < depth; z += 1)
            for (let x = 0; x < width; x += 1)
                surfaces[z][x] = region.surfaceBlocks[z][x];
    }
    if (region.subsurfaceBlocks)
        validateMatrix(region.subsurfaceBlocks, width, depth, "subsurfaceBlocks");
    ...
    for (const placement of region.blocks ?? []) {
        ...
        if (isWater(placement.block))
            water.add(column);
        if (isVegetation(placement.block))
            vegetation.add(column);
        if (isPath(placement.block))
            paths.add(column);
    }
    for (const point of region.waterCoordinates ?? [])
        water.add(key2(point.x, point.z));
    for (const point of region.vegetationCoordinates ?? [])
        vegetation.add(key2(point.x, point.z));
    for (const point of region.pathCoordinates ?? [])
        paths.add(key2(point.x, point.z));
    const protectedCoordinates = [...(region.protectedCoordinates ?? [])];
    for (const placement of region.blocks ?? [])
        if (isSensitive(placement))
            protectedCoordinates.push({ x: placement.x, y: placement.y, z: placement.z });
    return {
        heights,
        biomes,
        surfaces,
...
function matrixValue(matrix, region, x, z) {
...
function transformColumn(localX, localZ, width, depth, rotation, mirrorX) {
    const x = mirrorX ? width - 1 - localX : localX;
    if (rotation === 90)
        return { x: depth - 1 - localZ, z: x };
    if (rotation === 180)
        return { x: width - 1 - x, z: depth - 1 - localZ };
    if (rotation === 270)
        return { x: localZ, z: width - 1 - x };
    return { x, z: localZ };
}
...
function candidateOffsets(maximum) {
...
function candidateBaseY(strategy, heights) {
    const middle = median(heights);
    const maximum = Math.max(...heights);
    const minimum = Math.min(...heights);
    if (strategy === "raised_foundation")
        return middle + 2;
    if (strategy === "stilts" || strategy === "bridge_span")
        return maximum + 2;
    if (strategy === "sunken")
        return Math.max(minimum + 1, middle);
    if (strategy === "cliff_embedded")
        return Math.max(minimum + 1, middle);
    return middle + 1;
}
function candidateId(anchor, rotation, mirrorX) {
    return `candidate_${digest({ anchor, rotation, mirrorX }).slice(0, 12)}`;
}
function nearestPathDistance(grid, x, z) {
    if (!grid.paths.size)
        return 0;
...
function evaluateCandidate(build, region, grid, terrainInterface, anchor, rotation, mirrorX) {
    ...
    const columns = [];
    for (let localZ = 0; localZ < sourceDepth; localZ += 1)
        for (let localX = 0; localX < sourceWidth; localX += 1) {
    ...
    for (const column of columns) {
        ...
        if (delta < 0) {
            ...
            if (-delta > terrainInterface.maxCutDepth)
                cutLimitViolations += 1;
        }
        else if (delta > 0) {
            fill += delta;
            foundationExposure += 1;
            maximumFill = Math.max(maximumFill, delta);
            if (delta > terrainInterface.maxFillHeight)
                fillLimitViolations += 1;
        }
        if (grid.water.has(key2(column.x, column.z)) && !["waterfront", "bridge_span"].includes(terrainInterface.strategy))
            waterConflicts += 1;
    }
    ...
    for (const point of grid.protected)
        if (insideBounds(point, candidateBounds))
            protectedConflicts += 1;
    for (const item of grid.protectedRegions)
        if (boxesOverlap(candidateBounds, item.bounds))
            protectedConflicts += 1;
...
function distanceToRectangle(x, z, minX, maxX, minZ, maxZ) {
...
function regularization(seed, x, z) {
    const value = Number.parseInt(digest(`${seed}:${x}:${z}`).slice(0, 8), 16) / 0xffff_ffff;
    return value - 0.5;
}
function strataForBiome(biome, observedColumns = []) {
    ...
    if (maximumObservedDepth) {
        ...
        for (let depth = 0; depth < maximumObservedDepth; depth += 1) {
            const layer = observedColumns.map((column) => column[depth]).filter((block) => Boolean(block) && !isAir(block));
            const mostCommon = materialCounts(layer)[0]?.block;
            if (mostCommon)
                consensus.push(mostCommon);
        }
        if (consensus.length)
            return consensus;
    }
    if (/desert|badlands/.test(biome))
        return ["minecraft:sand", "minecraft:sand", "minecraft:sandstone", "minecraft:stone"];
    if (/taiga|old_growth|snow/.test(biome))
        return ["minecraft:podzol", "minecraft:dirt", "minecraft:stone"];
    if (/swamp|mangrove/.test(biome))
        return ["minecraft:mud", "minecraft:mud", "minecraft:clay", "minecraft:stone"];
    if (/beach|ocean/.test(biome))
        return ["minecraft:sand", "minecraft:sandstone", "minecraft:stone"];
    return ["minecraft:grass_block", "minecraft:dirt", "minecraft:dirt", "minecraft:stone"];
}
function materialCounts(values) {
    const counts = new Map();
    for (const value of values)
        counts.set(value, (counts.get(value) ?? 0) + 1);
    return [...counts.entries()].map(([block, count]) => ({ block, count })).sort((a, b) => b.count - a.count || a.block.localeCompare(b.block));
}
function biomeCounts(values) {
    const counts = new Map();
    for (const value of values)
        counts.set(value, (counts.get(value) ?? 0) + 1);
    return [...counts.entries()].map(([biome, count]) => ({ biome, count })).sort((a, b) => b.count - a.count || a.biome.localeCompare(b.biome));
}
function retainingPalette(style) {
    if (/japan|temple|pagoda/.test(style))
        return { material: "minecraft:stone_bricks", accentMaterial: "minecraft:mossy_stone_bricks" };
    if (/modern|brutalist/.test(style))
        return { material: "minecraft:smooth_stone", accentMaterial: "minecraft:gray_concrete" };
    return { material: "minecraft:cobblestone", accentMaterial: "minecraft:mossy_cobblestone" };
}
function pointSide(x, z, bounds) {
    if (z === bounds.minZ)
        return "north";
    if (z === bounds.maxZ)
        return "south";
...
function routeToPath(region, grid, candidate, waterPolicy, searchLimit) {
    if (!grid.paths.size) {
        const start = {
    ...
    while (queue.length && visited.size <= maximum) {
        ...
        for (const next of [
            ...
            if (nextHeight === undefined || currentHeight === undefined || Math.abs(nextHeight - currentHeight) > 2)
                continue;
            if (waterPolicy === "preserve" && grid.water.has(nextKey))
                continue;
            if (grid.protected.some((point) => point.x === next.x && point.z === next.z))
                continue;
            if (grid.protectedRegions.some((item) => next.x >= item.bounds.min.x && next.x <= item.bounds.max.x && next.z >= item.bounds.min.z && next.z <= item.bounds.max.z))
                continue;
            visited.add(nextKey);
...
function terrainPreviewHash(preview) {
    return digest(previewPayload(preview));
}
export function analyzeTerrainFit(build, region, options) {
    if (build.input.edition !== region.edition)
        throw new Error(`TERRAIN_FIT_EDITION_MISMATCH: build is ${build.input.edition}, region is ${region.edition}.`);
    if (build.input.version !== region.version)
        throw new Error(`TERRAIN_FIT_VERSION_MISMATCH: build is ${build.input.version}, region is ${region.version}.`);
    ...
    if (sourceColumns > maximumFootprintColumns)
        throw new Error(`TERRAIN_FIT_LIMIT: build footprint has ${sourceColumns.toLocaleString()} columns; limit is ${maximumFootprintColumns.toLocaleString()}.`);
    ...
    const rotations = [...new Set(options.rotations?.length ? options.rotations : [0, 90, 180, 270])]
        .filter((rotation) => [0, 90, 180, 270].includes(rotation));
    if (!rotations.length)
        throw new Error("TERRAIN_FIT_INVALID: at least one supported rotation is required.");
    ...
    candidateSearch: for (const offset of offsets)
    ...
    if (!candidates.length)
        throw new Error("TERRAIN_FIT_NO_CANDIDATE: no candidate footprint fits inside the supplied world region.");
    ...
    const deltas = [];
    const retainingColumns = [];
    const sampledSurface = [];
    const sampledBiomeValues = [];
    const sampledSubsurfaceByBiome = new Map();
    ...
    for (let z = scanMinZ; z <= scanMaxZ; z += 1)
        for (let x = scanMinX; x <= scanMaxX; x += 1) {
            ...
            if (distance > terrainInterface.blendRadius)
                continue;
            ...
            const zone = distance === 0 ? "footprint" : distance <= (terrainInterface.innerBlendRadius ?? 0) ? "inner" : "outer";
            if (zone === "footprint")
                footprintColumns += 1;
            else if (zone === "inner")
                innerColumns += 1;
    ...
    for (const protectedRegion of grid.protectedRegions)
        if (boxesOverlap(selectedBounds, protectedRegion.bounds))
            protectedConflicts.push({ ...protectedRegion.bounds.min });
    ...
    if (retainingColumns.length && terrainInterface.allowRetainingWalls)
        operations.push({ op: "retaining_structure", style: build.input.style, ...retainingPalette(build.input.style), columns: retainingColumns });
    if (pathConnection.status === "connected")
        operations.push({
            op: "connect_path",
    ...
    if (waterConflicts.length)
        operations.push({ op: "water_interface", policy: terrainInterface.waterPolicy ?? "preserve", coordinates: waterConflicts });
    ...
    if (deltas.length && vegetationDensity > 0)
        operations.push({
    ...
    const warnings = [];
    if (maximumCutDepth > terrainInterface.maxCutDepth)
        warnings.push(`Maximum cut depth ${maximumCutDepth} exceeds the configured limit ${terrainInterface.maxCutDepth}.`);
    if (maximumFillHeight > terrainInterface.maxFillHeight)
        warnings.push(`Maximum fill height ${maximumFillHeight} exceeds the configured limit ${terrainInterface.maxFillHeight}.`);
    if (protectedConflicts.length)
        warnings.push(`${protectedConflicts.length} protected conflict(s) remain at the selected placement.`);
    if (structureConflicts.length)
        warnings.push(`${structureConflicts.length} existing structure conflict(s) remain at the selected placement.`);
    if (waterConflicts.length && terrainInterface.waterPolicy === "preserve")
        warnings.push(`${waterConflicts.length} water column(s) intersect the footprint and must be preserved or the placement changed.`);
    if (pathConnection.status === "blocked")
        warnings.push("A nearby path exists, but no bounded route avoids protected terrain, water policy, and steep steps.");
    if (retainingColumns.length && !terrainInterface.allowRetainingWalls)
        warnings.push("Retaining support is required but disabled.");
    const risk = protectedConflicts.length || structureConflicts.length
...
export function verifyTerrainFitPreview(preview) {
...
export function confirmTerrainFitPreview(preview, confirmationHash) {
    verifyTerrainFitPreview(preview);
    if (confirmationHash !== preview.hash)
        throw new Error("TERRAIN_FIT_CONFIRMATION_MISMATCH: confirmation must name the exact immutable preview hash.");
    if (preview.risk === "red")
        throw new Error("TERRAIN_FIT_BLOCKED: red-risk previews cannot be confirmed until their conflicts or configured limits are resolved.");
    return {
        schemaVersion: 1,
...
//# sourceMappingURL=terrain-fit.js.map