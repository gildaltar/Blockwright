...
function configuredPort() {
    ...
    if (!/^\d{1,5}$/.test(raw))
        throw new Error("Blockwright local runtime port must be an integer from 1 through 65535.");
...
export function isStrongLocalMcpToken(token) {
...
function expectedAuthorities(port) {
    const suffix = port === 80 ? "" : `:${port}`;
    return new Set([`127.0.0.1${suffix}`, `localhost${suffix}`]);
}
function isAllowedAuthority(value, port) {
...
export function trustedClientAddress(request, trustProxyHops = 0) {
    ...
    if (addresses.length < trustProxyHops || addresses.some((value) => isIP(value) === 0))
        return socketAddress;
...
function requestPathFromTarget(target) {
    return (target ?? "/").split("?", 1)[0];
}
function requestPath(request) {
    return requestPathFromTarget(request.url);
}
export function isCanonicalMcpRequestTarget(target) {
...
function couldReachExpressMcpMount(path) {
    let candidate = path;
...
export function mcpRequestTargetDisposition(target) {
...
function canonicalizeMcpRequestTarget(request) {
...
function guardedJsonPath(request) {
    const path = requestPath(request);
    return path === "/mcp" || path === "/api" || path.startsWith("/api/");
}
function reject(response, statusCode, message, authenticate = false, extra = {}) {
    ...
    if (authenticate)
        response.setHeader("www-authenticate", "Bearer realm=\"Blockwright local MCP\"");
...
function rejectHostedVerification(response, error) {
    const guarded = error;
    if (guarded.code === "HOSTED_MCP_RATE_LIMITED") {
        ...
        reject(response, 429, "Too many hosted MCP requests. Try again after the rate-limit reset time.", false, guarded.limit ? { resetAt: guarded.limit.resetAt } : {});
        return;
    }
...
function createProductionRequestBoundary(application, port, options, token) {
    ...
    const activeJsonBodiesByClient = new Map();
    let jsonWindowStartedAt = Date.now();
    let jsonBodiesStarted = 0;
    const jsonBodiesStartedByClient = new Map();
    return (request, response) => {
        ...
        if (isMcpRequest)
            canonicalizeMcpRequestTarget(request);
        let releaseBodySlot;
            ...
            if (length > 0) {
                ...
                const incompleteBodyTimer = setTimeout(() => {
                    ...
                    if (!response.headersSent)
                        reject(response, 408, "The JSON request body was not completed within the ingress timeout.");
                    request.destroy();
                    releaseBodySlot?.();
                ...
                releaseBodySlot = () => {
                    ...
                    if (remainingForClient > 0)
                        activeJsonBodiesByClient.set(clientAddress, remainingForClient);
                    else
                        activeJsonBodiesByClient.delete(clientAddress);
                };
...
function installShutdown(httpServer) {
    const shutdown = () => {
...
async function initializeAsHandler(server) {
...
async function runProductionNode(server, options) {
    ...
    await new Promise((resolve, rejectPromise) => {
        ...
        if (options.hostedMode)
            ...
            httpServer.listen(port, LOCAL_BIND_ADDRESS, onListening);
    });
    installShutdown(httpServer);
    if (!options.hostedMode)
        console.log(`Blockwright listening on http://${LOCAL_BIND_ADDRESS}:${port}`);
    return undefined;
}
async function runLocalDevelopment(server) {
    const port = configuredPort();
    ...
    try {
        ...
        if (!intercepted)
            throw new Error("Blockwright could not enforce the local development loopback listener.");
        return result;
    }
    finally {
        if (ownDescriptor)
            Object.defineProperty(HttpServer.prototype, "listen", ownDescriptor);
        else
            delete HttpServer.prototype.listen;
    }
}
export async function runBlockwrightRuntime(server, options) {
    ...
    if (process.env.NODE_ENV === "production")
        return runProductionNode(server, options);
    if (options.hostedMode)
        return server.run();
    return runLocalDevelopment(server);
}
//# sourceMappingURL=runtime-listener.js.map