...
async function parseHostedImport(input) {
    if (input.bytes.byteLength > input.maximumCompressedBytes)
        throw new Error(`Schematic input exceeds the configured ${input.maximumCompressedBytes}-byte compressed limit.`);
    if (input.bytes[0] !== 0x1f || input.bytes[1] !== 0x8b)
        throw new Error("Schematic input must be gzip-compressed Java NBT.");
    const expanded = await new Promise((resolveExpanded, rejectExpanded) => {
        gunzip(Buffer.from(input.bytes), { maxOutputLength: input.maximumExpandedBytes }, (error, result) => {
            if (error)
                rejectExpanded(error);
            else
                resolveExpanded(result);
        });
    });
    if (expanded.byteLength > input.maximumExpandedBytes)
        throw new Error(`Schematic input exceeds the configured ${input.maximumExpandedBytes}-byte expanded limit.`);
    const limits = {
