...
const DYE_NAMES = Object.keys(DYE_COLORS).sort((left, right) => right.length - left.length);
function stateValue(placement, key) {
...
function cardinalValue(value, fallback = "north") {
    if (typeof value !== "string")
        return fallback;
    return cardinalDirections.has(value) ? value : fallback;
}
function numericDirection(value, fallback = "north") {
 ...
 */
export function resolveReviewerPlacementState(placement, edition) {
...
export function reviewerBlockColor(block) {
    ...
    if (colorable) {
        ...
        if (dye) {
            ...
            if (/terracotta/.test(id) && !/glazed/.test(id))
                return mixHex(base, "#7d4c3d", .36);
            if (/glazed_terracotta/.test(id))
                return mixHex(base, "#f0e2d3", .12);
            if (/wool|carpet/.test(id))
                return mixHex(base, "#ebe7dd", .08);
            if (/stained_glass/.test(id))
                return mixHex(base, "#d9f1f2", .18);
            if (/concrete_powder/.test(id))
                return mixHex(base, "#eadfcf", .1);
            return base;
        }
    }
    if (/water|bubble_column/.test(id))
        return "#2387cf";
    if (/packed_ice|blue_ice/.test(id))
        return "#68b7e6";
    if (/ice/.test(id))
        return "#a4d8e5";
    if (/glass|pane/.test(id))
        return "#a7d2d7";
    if (/sea_lantern|lantern|glowstone|froglight|shroomlight|ochre_froglight/.test(id))
        return "#efba55";
    if (/redstone|magma/.test(id))
        return "#b84332";
    if (/diamond/.test(id))
        return "#55d7ce";
    if (/emerald/.test(id))
    ...
    if (/gold/.test(id))
        return "#d7ac35";
    if (/netherite/.test(id))
        return "#39353a";
    if (/oxidized_copper/.test(id))
        return "#4f9a83";
    if (/weathered_copper/.test(id))
        return "#65927c";
    if (/exposed_copper/.test(id))
        return "#ad7958";
    if (/copper/.test(id))
        return "#b96342";
    if (/iron|chain|anvil|hopper|cauldron/.test(id))
        return "#8b9292";
    if (/deepslate|blackstone|coal|basalt/.test(id))
        return "#30383d";
    if (/quartz|calcite|diorite|bone_block/.test(id))
        return "#d8d3c6";
    if (/prismarine/.test(id))
        return "#559687";
    if (/sandstone|sand|end_stone/.test(id))
        return "#cbb67b";
    if (/granite|brick/.test(id))
        return "#8d5545";
    if (/stone|tuff|andesite|cobble|gravel/.test(id))
        return "#747a78";
    if (/warped/.test(id))
        return "#347b76";
    if (/crimson/.test(id))
    ...
    if (/cherry/.test(id))
        return "#d39a9a";
    if (/spruce|dark_oak/.test(id))
        return "#49311f";
    if (/jungle|acacia/.test(id))
        return "#9a5d36";
    if (/oak|bamboo|birch/.test(id))
        return "#ad8250";
    if (/moss|grass|leaves|vine|cactus|azalea|lily|fern/.test(id))
        return "#527b4d";
    if (/netherrack|nether_brick/.test(id))
        return "#63343b";
    if (/snow/.test(id))
        return "#e6eff0";
    if (/clay/.test(id))
...
export function reviewerGeometryParts(placement, edition) {
    ...
    if (/_fence$|_wall$/.test(id)) {
        const connected = (side) => ["true", "low", "tall"].includes(String(placement.state?.[side] ?? ""));
    ...
    if (/_door$/.test(id) && !/_trapdoor$/.test(id)) {
        ...
        if (!state.open)
            return [{ size: [1, 1, .1875], offset: [0, 0, 0], rotationY: closedYaw }];
    ...
    if (/(^|:)lantern$|soul_lantern$/.test(id)) {
        return [
            ...
            ...(state.hanging ? [{ size: [.12, .28, .12], offset: [0, .43, 0], role: "metal" }] : []),
        ];
    }
    return [{ size: [1, 1, 1], offset: [0, 0, 0] }];
}
//# sourceMappingURL=reviewer-rendering.js.map