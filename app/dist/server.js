...
const rolePaletteSchema = z.object(Object.fromEntries(paletteRoles.map((role) => [role, z.string()])));
...
function asBuild(value) {
    const reference = typeof value === "string" ? value : value && typeof value === "object" && "id" in value ? String(value.id) : undefined;
    if (reference && buildCache.has(reference))
        return buildCache.get(reference);
    ...
    return rememberBuild(compileBuild(value.input));
}
...
const server = new McpServer({ name: "blockwright", version: "0.4.0" }, { capabilities: {} })
    .registerTool({
