...
function asBuild(value) {
    ...
    const compiled = compileBuild(value.input);
...
function cachedBuildForView(value) {
    ...
    if (!suppliedId)
        throw new Error("VIEW_BUILD_ID_REQUIRED: placement pages require the build id returned by compile_build or review_build.");
...
function buildViewMetadata(build) {
...
function hostedImportLimitError(error, format) {
    if (!hostedConfig.enabled || !(error instanceof Error) || !/volume.*limit|occupied placements.*cap/i.test(error.message))
        return undefined;
...
const server = new McpServer({ name: APP_NAME, version: APP_VERSION }, { capabilities: {} }, {
    ...
    const candidates = generateBuildCandidates(buildInput, candidateCount, (recentPlans ?? []));
    ...
    const build = rememberBuild(compileBuild(input));
    cacheBuildForView(build);
    ...
    if (selectedRegionMode) {
        ...
        return {
            structuredContent: { mode: "selected_region", previousHash: current.hash, build: summarizeBuild(next), region: result.region, preservedOutsideCount: result.preservedOutsideCount, diff },
    ...
    cacheBuildForView(next);
    return {
        structuredContent: { mode: "whole_build", previousHash: current.hash, build: summarizeBuild(next) },
...
server.useOnError("/mcp", (error, _request, response, next) => {
    if (hostedConfig.enabled && error instanceof Error && error.code === HOSTED_MCP_PATH_ERROR) {
        response.setHeader("cache-control", "no-store");
        response.setHeader("x-content-type-options", "nosniff");
        response.status(404).json({ ok: false, error: error.message });
        return;
    }
    if (hostedConfig.enabled && error instanceof Error && error.code === HOSTED_MCP_PAYMENT_ERROR) {
        response.setHeader("cache-control", "no-store");
        response.setHeader("x-content-type-options", "nosniff");
        response.status(402).json({ ok: false, error: error.message });
        return;
    }
    if (hostedConfig.enabled && error instanceof Error && error.code === HOSTED_MCP_RATE_ERROR) {
    ...
    if (hostedConfig.enabled && error instanceof Error && error.code === HOSTED_MCP_AUTH_ERROR) {
        response.setHeader("cache-control", "no-store");
        response.setHeader("x-content-type-options", "nosniff");
        response.setHeader("www-authenticate", "Bearer realm=\"Blockwright\"");
        response.status(401).json({ ok: false, error: "Sign in to use hosted Blockwright MCP." });
