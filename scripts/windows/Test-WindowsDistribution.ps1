...
try {
    ...
    foreach ($required in @("previousDefaultPresent", "extensionKeyCreated", 'DeleteValue("", $false)', "SubKeyCount", "ValueCount")) { Assert-Condition ($associationSource.Contains($required)) "Association ownership contract is missing: $required" }
    Add-Pass "reversible-schem-association" "Removal restored only the prior default association and preserved unrelated extension subkeys and values."
