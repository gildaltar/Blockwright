...
$proof = [ordered]@{ schemaVersion = 1; file = [System.IO.Path]::GetFileName($artifactPath); sha256 = $hash; status = "Valid"; signerThumbprint = $actual; signerSubject = [string]$signature.SignerCertificate.Subject; timestampCertificateSubject = if ($null -eq $signature.TimeStamperCertificate) { $null } else { [string]$signature.TimeStamperCertificate.Subject } }
