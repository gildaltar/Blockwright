...
const bedrockManifestVersionSchema = z.array(
  z.number().int().min(0).max(65_535).describe("One Bedrock manifest version component."),
).min(3).max(3).describe("Exactly three non-negative integers in major, minor, patch order.");
    ...
    async ({ builds: values, name, description, packId, manifestVersion }) => {
      ...
      try {
        const mcpack = await createBedrockMcpack({
          ...
          packId,
          builds,
        }, { manifestVersion: manifestVersion as [number, number, number] });
        assertHostedArtifactOutputSize(mcpack.bytes.byteLength);
        return {
          structuredContent: {
