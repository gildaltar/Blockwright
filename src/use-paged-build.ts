...
export function usePagedBuild(summary: BuildSummary | undefined, initialPage?: BuildPlacementPage, legacyBuild?: BuildRecord): PagedBuildState {
  const { callToolAsync } = useCallTool("get_build_chunk");
  const callToolRef = useRef(callToolAsync);
  callToolRef.current = callToolAsync;
  const [state, setState] = useState<PagedBuildState>({ loaded: 0, total: summary?.blockCount ?? 0, isLoading: Boolean(summary) });
  ...
  useEffect(() => {
    ...
    void loadBuildForView({
      ...
      if (!cancelled) setState({ loaded: 0, total: summary.blockCount, isLoading: false, error: error instanceof Error ? error.message : "Build placements could not be loaded." });
    });
