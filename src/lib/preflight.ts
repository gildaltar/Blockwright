...
export function preflightConfirmationToken(input: BuildInput) {
...
export function estimateBuild(input: BuildInput, overrides: Partial<RiskThresholds> = {}): BuildPreflight {
  ...
  ].filter((value): value is string => typeof value === "string" && Boolean(value))).size || 11;
  ...
  if (estimatedPlacementAttempts > thresholds.hardAttemptLimit) warnings.push(`The generic program may attempt ${estimatedPlacementAttempts.toLocaleString()} coordinate operations, above Blockwright's ${thresholds.hardAttemptLimit.toLocaleString()} generation-work limit; simplify repeated or overlapping operations.`);
