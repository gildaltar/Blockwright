...
import type {
  ...
  DesignElement,
  DesignMaterial,
  DesignOperationPhase,
  ProceduralMaterialDefinition,
  DesignProgramV2,
  DesignRepetition,
  DesignTemplateInstance,
  Dimensions,
  Edition,
  RolePalette,
  Vec3,
...
const PHASE_ORDER: readonly DesignOperationPhase[] = [
  "terrain_foundation", "primary_mass", "structure", "walls", "roof", "cuts_openings",
  "trim", "detail", "lighting", "landscaping", "explicit_overrides",
];
const PHASES = new Set<DesignOperationPhase>(PHASE_ORDER);

export type ResolvedComponentElement = {
...
export type ResolvedComponent = {
  ...
  elements: ResolvedComponentElement[];
  manifest: ComponentGraphEntry;
};
...
export function resolveComponentGraph(
  ...
  if (!design.components.length) throw new Error("DESIGN_COMPONENTS_EMPTY: Design IR v2 must contain at least one component.");

  for (const template of design.templates) {
    if (!template.elementIds.length) throw new Error(`DESIGN_TEMPLATE_EMPTY: ${template.id} must reference at least one element.`);
    assertUniqueReferences(template.elementIds, `template ${template.id}`);
    for (const elementId of template.elementIds) if (!elements.has(elementId)) throw new Error(`DESIGN_TEMPLATE_ELEMENT_UNKNOWN: ${template.id} references ${elementId}.`);
  }
  for (const component of design.components) {
    if (!component.name.trim() || !component.type.trim() || !component.seed.trim()) throw new Error(`DESIGN_COMPONENT_METADATA_INVALID: ${component.id} needs non-empty name, type, and seed values.`);
    if (!PHASES.has(component.operationPhase)) throw new Error(`DESIGN_COMPONENT_PHASE_INVALID: ${component.id} uses ${String(component.operationPhase)}.`);
    ...
    for (const dependency of component.dependencies) {
      if (!components.has(dependency)) throw new Error(`DESIGN_COMPONENT_DEPENDENCY_UNKNOWN: ${component.id} depends on ${dependency}.`);
      if (dependency === component.id) throw new Error(`DESIGN_COMPONENT_DEPENDENCY_CYCLE: ${component.id} depends on itself.`);
    }
    for (const elementId of component.elementIds) if (!elements.has(elementId)) throw new Error(`DESIGN_COMPONENT_ELEMENT_UNKNOWN: ${component.id} references ${elementId}.`);
    const instanceIds = new Set<string>();
    for (const instance of component.templateInstances ?? []) {
      assertId(instance.id, "template instance");
      if (instanceIds.has(instance.id)) throw new Error(`DESIGN_TEMPLATE_INSTANCE_ID_DUPLICATE: ${component.id}.${instance.id}`);
      instanceIds.add(instance.id);
      if (!templates.has(instance.templateId)) throw new Error(`DESIGN_TEMPLATE_UNKNOWN: ${component.id}.${instance.id} references ${instance.templateId}.`);
      if (instance.origin) assertIntegerPoint(instance.origin, `${component.id}.${instance.id}.origin`);
  ...
  const combinedById = new Map<string, string>();
  const resolvedById = new Map<string, ResolvedComponent>();
  for (const componentId of order) {
    const definition = components.get(componentId)!;
    const resolvedElements: ResolvedComponentElement[] = definition.elementIds.map((elementId) => ({
    ...
    const manifest: ComponentGraphEntry = {
      id: definition.id,
  ...
  const manifestWithoutHash = { schemaVersion: 1 as const, order, components: resolved.map(({ manifest }) => manifest) };
...
function topologicalPhaseOrder(definitions: DesignComponent[]) {
  ...
  for (const component of definitions) for (const dependency of component.dependencies) {
...
function repetitionTransforms(instance: DesignTemplateInstance): InstanceTransform[] {
  const origin = instance.origin ?? { x: 0, y: 0, z: 0 };
  const repetition = instance.repetition;
  if (!repetition) return [{ translation: { ...origin } }];
  if (repetition.kind === "linear") return Array.from({ length: repetition.count }, (_, index) => ({ translation: add(origin, scale(repetition.step, index)) }));
  if (repetition.kind === "grid") {
    const transforms: InstanceTransform[] = [];
    for (let y = 0; y < repetition.count.y; y += 1) for (let z = 0; z < repetition.count.z; z += 1) for (let x = 0; x < repetition.count.x; x += 1) {
      transforms.push({ translation: add(origin, { x: repetition.step.x * x, y: repetition.step.y * y, z: repetition.step.z * z }) });
    }
    return transforms;
  }
  if (repetition.kind === "radial") return Array.from({ length: repetition.count }, (_, index) => {
  ...
  if (repetition.kind === "alternating") return Array.from({ length: repetition.count }, (_, index) => ({ translation: add(
    add(origin, scale(repetition.step, index)),
    index % 2 ? repetition.alternateOffset : { x: 0, y: 0, z: 0 },
  ) }));
  return repetition.positions.map((position) => ({ translation: add(origin, position) }));
}

function validateRepetition(repetition: DesignRepetition | undefined, label: string) {
  if (!repetition) return;
  if (repetition.kind === "position_list") {
    if (!repetition.positions.length) throw new Error(`DESIGN_REPETITION_INVALID: ${label} position_list must not be empty.`);
    repetition.positions.forEach((point, index) => assertIntegerPoint(point, `${label}.positions[${index}]`));
    return;
  }
  if (repetition.kind === "mirrored") {
    if (!Number.isSafeInteger(repetition.coordinate)) throw new Error(`DESIGN_REPETITION_INVALID: ${label}.coordinate must be an integer.`);
    return;
  }
  if (repetition.kind === "grid") {
    for (const axis of ["x", "y", "z"] as const) if (!Number.isSafeInteger(repetition.count[axis]) || repetition.count[axis] < 1) {
  ...
  if (repetition.kind === "radial" && (!Number.isFinite(repetition.radius) || repetition.radius < 0)) throw new Error(`DESIGN_REPETITION_INVALID: ${label}.radius must be non-negative.`);
  if (repetition.kind === "radial") {
    assertIntegerPoint(repetition.center, `${label}.center`);
    if (repetition.startAngleDegrees !== undefined && !Number.isFinite(repetition.startAngleDegrees)) throw new Error(`DESIGN_REPETITION_INVALID: ${label}.startAngleDegrees must be finite.`);
  }
  if ("step" in repetition) assertIntegerPoint(repetition.step, `${label}.step`);
  if (repetition.kind === "alternating") assertIntegerPoint(repetition.alternateOffset, `${label}.alternateOffset`);
}

function transformElement(element: DesignElement, transform: InstanceTransform, overrides: Record<string, string>): DesignElement {
  const translated = cloneElement(element);
  const translatedPoint = (point: Vec3) => reflectPoint(add(point, transform.translation), transform.mirror);
  if (translated.kind === "fill" || translated.kind === "shell" || translated.kind === "carve" || translated.kind === "basin") {
    ...
    if (translated.kind === "basin" && translated.liquidLevel !== undefined) translated.liquidLevel += transform.translation.y;
  } else if (translated.kind === "cylinder") translated.center = translatedPoint(translated.center);
  else if (translated.kind === "sweep") {
    translated.points = translated.points.map(translatedPoint);
    if (translated.supports) translated.supports = { ...translated.supports, toY: translated.supports.toY + transform.translation.y };
    ...
    if (translated.masks) translated.masks = translated.masks.map((mask) => ({
      ...mask,
  ...
  if (translated.offsets) translated.offsets = translated.offsets.map((offset) => transform.mirror?.axis === "x"
    ? { ...offset, x: -offset.x }
    : transform.mirror?.axis === "z" ? { ...offset, z: -offset.z } : offset);
  return overrideMaterials(translated, overrides);
}

function reflectPoint(point: Vec3, mirror: InstanceTransform["mirror"]): Vec3 {
...
function overrideMaterials(element: DesignElement, overrides: Record<string, string>): DesignElement {
  const replace = (reference: string | undefined) => reference === undefined ? undefined : overrides[reference] ?? reference;
  if (element.kind === "carve") return element;
  if (element.kind === "procedural") return { ...element, material: replace(element.material) };
  if (element.kind === "basin") return {
    ...element,
  ...
  if (element.kind === "stairs" || element.kind === "ramp") return { ...element, material: replace(element.material)!, railingMaterial: replace(element.railingMaterial) };
  return { ...element, material: replace(element.material)! };
}

function materialReferences(element: DesignElement) {
  if (element.kind === "carve") return [];
  if (element.kind === "procedural") return element.material ? [element.material] : [];
  if (element.kind === "basin") return [element.wallMaterial, element.floorMaterial, element.rimMaterial, element.liquidMaterial].filter((value): value is string => Boolean(value));
  if (element.kind === "sweep") return [element.material, element.innerMaterial, element.supports?.material].filter((value): value is string => Boolean(value));
  if (element.kind === "stairs" || element.kind === "ramp") return [element.material, element.railingMaterial].filter((value): value is string => Boolean(value));
  return [element.material];
}

function withoutMaterialFields(element: DesignElement) {
  const clone = cloneElement(element) as unknown as Record<string, unknown>;
  for (const key of ["material", "wallMaterial", "floorMaterial", "rimMaterial", "liquidMaterial", "innerMaterial", "railingMaterial"]) delete clone[key];
  if (clone.supports && typeof clone.supports === "object") {
    const supports = { ...(clone.supports as Record<string, unknown>) };
...
function uniqueById<Value extends { id: string }>(values: Value[], label: string) {
  ...
  for (const value of values) {
    ...
    if (map.has(value.id)) throw new Error(`DESIGN_${label.toUpperCase()}_ID_DUPLICATE: ${value.id}`);
...
function assertId(id: string, label: string) {
  if (!ID_PATTERN.test(id)) throw new Error(`DESIGN_${label.toUpperCase().replace(/ /g, "_")}_ID_INVALID: ${id}`);
}

function assertUniqueReferences(values: string[], label: string) {
  if (new Set(values).size !== values.length) throw new Error(`DESIGN_REFERENCE_DUPLICATE: ${label} contains duplicate IDs.`);
}

function assertBounds(component: DesignComponent, dimensions: Dimensions) {
  ...
  if (min.x > max.x || min.y > max.y || min.z > max.z) throw new Error(`DESIGN_COMPONENT_BOUNDS_INVALID: ${component.id} bounds must be normalized.`);
...
function scale(point: Vec3, multiplier: number): Vec3 {
