import { auditBuildSemantics, validateBuildContract, type SemanticBuildAudit } from "./contract.js";
...
function binaryStateValue(value: unknown) {
  return typeof value === "boolean" || value === 0 || value === 1;
}

function binaryStateBoolean(value: unknown) {
...
function bedrockDoorState(placement: Placement) {
  ...
  return { facing: facing as keyof typeof directions, hinge, open, upper };
}

function isDoorPlacement(placement: Placement | undefined): placement is Placement {
  return Boolean(placement && /_door$/.test(placement.block) && !/_trapdoor$/.test(placement.block));
}

function bedrockStairFacing(placement: Placement) {
  const value = placement.state?.weirdo_direction;
  if (typeof value !== "number" || !Number.isInteger(value)) return undefined;
  return ({ 0: "east", 1: "west", 2: "south", 3: "north" } as const)[value as 0 | 1 | 2 | 3];
}
...
export function auditBuild(build: BuildRecord): BuildAudit {
  ...
  for (const placement of build.placements) {
    ...
    if (/_door$/.test(placement.block) && !/_trapdoor$/.test(placement.block)) {
      ...
      const valid = bedrock
        ...
        : Boolean(
          stateValue(placement, "facing") && stateValue(placement, "facing")! in directions
      ...
      if (!valid) {
        collect(
          ...
          bedrock
            ...
            : "Java doors must preserve cardinal facing, upper/lower half, left/right hinge, and boolean open state.",
          placement,
        );
      }
      if (nativeBedrockDoorState) {
        ...
        if (!compatible) {
          collect(
            findings,
            "INVALID_BEDROCK_DOOR_PAIR",
            "error",
            "Each Bedrock door half must have the opposite half directly above or below, using the same door block, cardinal direction, hinge side, and open state.",
            placement,
          );
        }
      }
  ...
  const semanticCodes: Record<string, string> = {
  ...
  for (const check of semantic.checks) {
    ...
    const severity: AuditFinding["severity"] = check.status === "fail" ? "error" : "warning";
    findings.set(code, {
      code,
      severity,
