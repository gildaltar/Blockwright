...
export type LitematicImportOptions = {
  origin?: Vec3;
...
const tag = {
  ...
  longArray: (value: bigint[]): Tag => ({ type: "longArray", value }),
...
function checkedText(value: string | undefined, fallback: string, maximum: number) {
  const result = (value ?? fallback).trim();
  if (!result || result.length > maximum) throw new Error(`Litematic text fields must contain between 1 and ${maximum} characters.`);
  return result;
}

function checkedVec3(value: Vec3, label: string) {
...
function primitiveTag(value: unknown): Tag {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) throw new Error("Block-entity NBT cannot contain non-finite numbers.");
    if (!Number.isInteger(value)) return { type: "double", value };
    if (value >= -2_147_483_648 && value <= 2_147_483_647) return tag.int(value);
    if (Number.isSafeInteger(value)) return tag.long(BigInt(value));
    throw new Error("Block-entity integer exceeds JavaScript's safe integer range.");
  }
  if (Array.isArray(value)) {
    if (!value.length) return tag.list("end", []);
    if (value.every((item) => Number.isInteger(item) && Number.isSafeInteger(item))) return tag.intArray(value as number[]);
    const entries = value.map(primitiveTag);
    if (!entries.every((entry) => entry.type === entries[0].type)) throw new Error("Block-entity NBT lists must contain one homogeneous tag type.");
...
function parseCanonicalState(value: string) {
  const bracket = value.indexOf("[");
  if (bracket < 0) return { block: value };
  if (!value.endsWith("]")) throw new Error(`Invalid canonical block state: ${value}`);
  const state = Object.fromEntries(value.slice(bracket + 1, -1).split(",").filter(Boolean).map((entry) => {
    const separator = entry.indexOf("=");
    if (separator < 1) throw new Error(`Invalid canonical block-state property: ${entry}`);
    return [entry.slice(0, separator), entry.slice(separator + 1)];
  }));
  return { block: value.slice(0, bracket), ...(Object.keys(state).length ? { state } : {}) };
}

function blockStateTag(value: string) {
  const parsed = parseCanonicalState(value);
  if (!/^[a-z0-9_.-]+:[a-z0-9_./-]+$/.test(parsed.block)) throw new Error(`Invalid Java block identifier in Litematic palette: ${parsed.block}`);
  return {
...
function blockStateFromTag(value: unknown) {
  if (!value || typeof value !== "object") throw new Error("Litematic palette entry must be a compound.");
  const entry = value as Record<string, unknown>;
  if (typeof entry.Name !== "string" || !/^[a-z0-9_.-]+:[a-z0-9_./-]+$/.test(entry.Name)) throw new Error("Litematic palette entry has an invalid block identifier.");
  const properties = entry.Properties;
  if (properties !== undefined && (!properties || typeof properties !== "object" || Array.isArray(properties))) throw new Error("Litematic block-state properties must be a compound.");
  const state = properties as Record<string, unknown> | undefined;
  if (state && !Object.values(state).every((item) => typeof item === "string")) throw new Error("Litematic block-state properties must be strings.");
  return canonicalState({ block: entry.Name, ...(state ? { state: state as Record<string, string> } : {}) });
}

function bitsPerEntry(paletteSize: number) {
  if (!Number.isSafeInteger(paletteSize) || paletteSize < 1) throw new Error("Litematic palette cannot be empty.");
  const bits = Math.max(2, Math.ceil(Math.log2(paletteSize)));
  if (bits > 32) throw new Error("Litematic palette exceeds the 32-bit entry limit.");
  return bits;
}

export function packLitematicPaletteIndices(values: number[], paletteSize: number) {
  const bits = bitsPerEntry(paletteSize);
  const mask = (1n << BigInt(bits)) - 1n;
  const unsigned = new Array<bigint>(Math.ceil(values.length * bits / 64)).fill(0n);
  for (let index = 0; index < values.length; index += 1) {
    const value = values[index];
    if (!Number.isSafeInteger(value) || value < 0 || value >= paletteSize) throw new Error(`Palette index ${value} is outside the palette.`);
...
function unsignedLong(value: unknown) {
  ...
  if (Array.isArray(value) && value.length === 2 && value.every(Number.isInteger)) {
    return BigInt.asUintN(64, (BigInt(value[0]) << 32n) | BigInt((value[1] as number) >>> 0));
  }
  throw new Error("Litematic BlockStates contains an invalid long value.");
}

export function unpackLitematicPaletteIndices(rawLongs: unknown[], count: number, paletteSize: number) {
  const bits = bitsPerEntry(paletteSize);
  const expectedLongs = Math.ceil(count * bits / 64);
  if (rawLongs.length !== expectedLongs) throw new Error(`Litematic BlockStates contains ${rawLongs.length} longs; expected ${expectedLongs}.`);
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (startArrayIndex !== endArrayIndex) value |= longs[endArrayIndex] << BigInt(64 - startBitOffset);
    const decoded = Number(value & mask);
    if (decoded >= paletteSize) throw new Error(`Litematic BlockStates references missing palette index ${decoded}.`);
    values.push(decoded);
  }
  return values;
}

function volume(dimensions: Dimensions) {
  return dimensions.width * dimensions.height * dimensions.depth;
}

function calculateBounds(placements: Placement[]) {
  const first = placements[0];
  if (!first) throw new Error("Cannot calculate Litematic bounds for an empty placement set.");
...
function epochMilliseconds(value: number | undefined, fallback: number) {
  const result = value ?? fallback;
  if (!Number.isSafeInteger(result) || result < 0) throw new Error("Litematic timestamps must be non-negative epoch milliseconds.");
  return result;
}

function uniquePlacementCoordinates(placements: Placement[]) {
  const seen = new Set<string>();
  for (const placement of placements) {
    checkedVec3(placement, "Placement");
    const key = `${placement.x},${placement.y},${placement.z}`;
    if (seen.has(key)) throw new Error(`Build contains duplicate placement coordinate ${key}.`);
    seen.add(key);
  }
}

export function createLitematicNbt(build: BuildRecord, options: LitematicOptions = {}) {
  if (build.input.edition !== "java") throw new Error(".litematic export is available for Java Edition builds only.");
  if (!build.placements.length) throw new Error("Cannot export an empty Litematic region.");
  ...
  const indices = new Array<number>(volume(bounds.dimensions)).fill(0);
  ...
  for (const placement of build.placements) {
    ...
    if (placement.block !== "minecraft:air") indices[index] = paletteIndex.get(canonicalState(placement))!;
...
export function exportLitematic(build: BuildRecord, options: LitematicOptions = {}) {
...
function readVec3(value: unknown, label: string): Vec3 {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`${label} must be a coordinate compound.`);
  const result = value as Record<string, unknown>;
  if (![result.x, result.y, result.z].every((coordinate) => Number.isInteger(coordinate))) throw new Error(`${label} must contain integer x, y, and z values.`);
  return checkedVec3({ x: result.x as number, y: result.y as number, z: result.z as number }, label);
}

function longAsNumber(value: unknown, label: string) {
  const result = typeof value === "bigint"
    ? value
    : Array.isArray(value) && value.length === 2 && value.every(Number.isInteger)
      ? (BigInt(value[0]) << 32n) | BigInt((value[1] as number) >>> 0)
      : undefined;
  if (result === undefined || result < 0n || result > BigInt(Number.MAX_SAFE_INTEGER)) throw new Error(`${label} is outside the supported timestamp range.`);
  return Number(result);
}

function regionMinimum(position: Vec3, size: Vec3): Vec3 {
  return {
...
function cloneBlockEntityData(entity: Record<string, unknown>) {
  const { id: _id, x: _x, y: _y, z: _z, ...data } = entity;
  return data;
}

export function importLitematic(bytes: Uint8Array, options: LitematicImportOptions = {}) {
  ...
  const maximumVolume = options.maximumVolume ?? 2_000_000;
  if (bytes.byteLength > compressedLimit) throw new Error(`Litematic input exceeds the configured ${compressedLimit}-byte compressed limit.`);
  if (bytes[0] !== 0x1f || bytes[1] !== 0x8b) throw new Error("Litematic input must be gzip-compressed Java NBT.");
  ...
  if (expanded.byteLength > expandedLimit) throw new Error(`Litematic input exceeds the configured ${expandedLimit}-byte expanded limit.`);
  const parsed = parseUncompressed(expanded, "big");
  const value = simplify(parsed) as Record<string, any>;
  const version = value.Version;
  if (version !== LITEMATIC_FORMAT_VERSION) throw new Error(`Only Litematic format v${LITEMATIC_FORMAT_VERSION} is supported; received ${String(version)}.`);
  if (!value.Regions || typeof value.Regions !== "object" || Array.isArray(value.Regions)) throw new Error("Litematic file does not contain a valid Regions compound.");
  ...
  for (const [name, rawRegion] of Object.entries(value.Regions as Record<string, any>).sort(([left], [right]) => left.localeCompare(right))) {
    if (!rawRegion || typeof rawRegion !== "object") throw new Error(`Litematic region ${name} must be a compound.`);
    const position = readVec3(rawRegion.Position, `Region ${name} Position`);
    const size = readVec3(rawRegion.Size, `Region ${name} Size`);
    if ([size.x, size.y, size.z].some((coordinate) => coordinate === 0)) throw new Error(`Litematic region ${name} has a zero dimension.`);
    ...
    if (!Array.isArray(rawRegion.BlockStatePalette) || !rawRegion.BlockStatePalette.length) throw new Error(`Litematic region ${name} has no block-state palette.`);
    ...
    const tileEntities = Array.isArray(rawRegion.TileEntities) ? rawRegion.TileEntities as Record<string, unknown>[] : [];
    ...
    for (const entity of tileEntities) {
      ...
      if (tileByCoordinate.has(tileKey)) throw new Error(`Region ${name} contains duplicate tile entity coordinate ${tileKey}.`);
      tileByCoordinate.set(tileKey, entity);
    }
    let blockCount = 0;
    for (let y = 0; y < dimensions.height; y += 1) for (let z = 0; z < dimensions.depth; z += 1) for (let x = 0; x < dimensions.width; x += 1) {
      ...
      if (occupied.has(key)) throw new Error(`Litematic regions overlap at occupied coordinate ${key}.`);
      occupied.add(key);
  ...
  if (!regions.length) throw new Error("Litematic file contains no regions.");
  ...
  return {
    format: "litematic" as const,
    version,
    subVersion: Number.isInteger(value.SubVersion) ? value.SubVersion as number : 0,
    minecraftDataVersion: Number.isInteger(value.MinecraftDataVersion) ? value.MinecraftDataVersion as number : undefined,
    metadata: {
      name: typeof metadata.Name === "string" ? metadata.Name : "",
      author: typeof metadata.Author === "string" ? metadata.Author : "",
      description: typeof metadata.Description === "string" ? metadata.Description : "",
      regionCount: Number.isInteger(metadata.RegionCount) ? metadata.RegionCount as number : regions.length,
      totalVolume: Number.isInteger(metadata.TotalVolume) ? metadata.TotalVolume as number : totalVolume,
      totalBlocks: Number.isInteger(metadata.TotalBlocks) ? metadata.TotalBlocks as number : placements.length,
    ...
    buildHash: typeof value.Blockwright?.BuildHash === "string" ? value.Blockwright.BuildHash as string : undefined,
    origin,
    bounds: {
      min: minimum,
      max: maximum,
