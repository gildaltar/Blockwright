...
import type { StripeBillingClient } from "./billing.js";
import { FixedWindowRateLimiter, type HostedPrincipal, type HostedServiceConfig, type HostedServiceStore, sessionTokenFromHeaders } from "./hosted-service.js";
import type { FileProjectStore, ProjectSnapshot, ProjectVersion, StoredReviewLink, createHostedReviewService } from "./projects.js";
...
type HandlerLike = (request: RequestLike, response: ResponseLike, next: () => void) => unknown;
type ExpressLike = {
  use(path: string, handler: HandlerLike): unknown;
  get(path: string, handler: HandlerLike): unknown;
  post(path: string, handler: HandlerLike): unknown;
  delete(path: string, handler: HandlerLike): unknown;
};

type HostedRouteDependencies = {
  config: HostedServiceConfig;
  store?: HostedServiceStore;
  billing?: StripeBillingClient;
  projectStore?: FileProjectStore;
  reviewService?: ReturnType<typeof createHostedReviewService>;
  resolveBuild?: (value: unknown, principal: HostedPrincipal) => BuildRecord;
};

const publicPrincipal = (principal: HostedPrincipal) => ({
...
const publicProjectVersion = (version: ProjectVersion) => {
...
const publicProjectSnapshot = (snapshot: ProjectSnapshot) => ({
...
function publicText(value: string, maximum: number) {
  return value.slice(0, maximum);
}

function publicMaterialState(state: string) {
  if (state.length <= PUBLIC_MATERIAL_STATE_CHARS) return { state };
  return {
...
function publicReviewBuild(build: BuildRecord) {
  const primary: [string, number][] = [];
  let materialStateCount = 0;
  for (const state in build.materialCounts) {
    if (!Object.hasOwn(build.materialCounts, state)) continue;
    ...
    if (primary.length > PUBLIC_REVIEW_PRIMARY_MATERIALS) primary.pop();
  }
...
function sendError(response: ResponseLike, error: unknown, status = 400) {
...
function routeError(message: string, httpStatus: number) {
...
function secureTokenEqual(left: string, right: string | undefined) {
...
export function configureHostedRoutes(app: ExpressLike, dependencies: HostedRouteDependencies) {
  ...
  const publicReviewVersionCache = new Map<string, { version: ProjectVersion; loadedAt: number; lastUsedAt: number }>();
  const publicReviewLoads = new Map<string, Promise<ProjectVersion>>();
  ...
  const projectReadsInFlightByTenant = new Map<string, number>();
  ...
  const unavailable = (response: ResponseLike) => {
  ...
  const principalFor = (request: RequestLike, response: ResponseLike) => {
    if (!store) {
  ...
  const writerFor = async (request: RequestLike, response: ResponseLike) => {
  ...
  const ownerFor = (request: RequestLike, response: ResponseLike) => {
    const authenticated = principalFor(request, response);
    if (!authenticated) return undefined;
  ...
  const canonicalBuild = (value: unknown, principal: HostedPrincipal) => {
    if (!resolveBuild) throw new Error("Canonical build validation is unavailable on this instance.");
    return resolveBuild(value, principal);
  };

  const pageValue = (value: unknown, fallback: number, minimum: number, maximum: number, label: string) => {
    const raw = value === undefined ? String(fallback) : typeof value === "string" ? value : "";
    if (!/^\d+$/.test(raw)) throw routeError(`${label} must be a non-negative integer.`, 400);
  ...
  const withProjectRead = async <T>(principal: HostedPrincipal, operation: () => Promise<T>) => {
    const tenantLimit = projectTenantReadWorkLimiter.consume(principal.tenantId);
    if (!tenantLimit.allowed) throw routeError("This workspace has reached its hosted project-read work limit. Retry after the rate-limit reset time.", 429);
    const globalLimit = projectReadWorkLimiter.consume("global");
    if (!globalLimit.allowed) throw routeError("Hosted project storage has reached its read-work limit. Retry after the rate-limit reset time.", 429);
    ...
    if (projectReadsInFlight >= 2) throw routeError("Hosted project storage is busy. Retry shortly.", 503);
  ...
  const reviewVersion = async (review: Pick<StoredReviewLink, "tenantId" | "projectId" | "versionId" | "buildHash">) => {
    if (!projectStore) throw routeError("Hosted project storage is unavailable.", 503);
    const now = Date.now();
    for (const [key, cached] of publicReviewVersionCache) {
      if (now - cached.loadedAt > publicReviewCacheTtlMs) publicReviewVersionCache.delete(key);
    }
    ...
    if (!tenantLoadLimit.allowed) throw routeError("This workspace's public review storage is busy. Retry after the rate-limit reset time.", 429);
    const loadLimit = publicReviewLoadLimiter.consume("global");
    if (!loadLimit.allowed) throw routeError("Public review storage is busy. Retry after the rate-limit reset time.", 429);
    if (publicReviewLoadsInFlight >= 1) throw routeError("Public review storage is busy. Retry shortly.", 503);
    const pending = (async () => {
      publicReviewLoadsInFlight += 1;
      try {
        const version = await projectStore.getVersion(review.tenantId, review.projectId, review.versionId);
        if (version.payload.build.hash !== review.buildHash) throw new Error("Review snapshot no longer matches its immutable build hash.");
  ...
  app.post("/api/auth/register", async (request, response) => {
    ...
    try {
      ...
      if (typeof email !== "string" || typeof password !== "string" || typeof tenantName !== "string") throw new Error("Email, password, and workspace name are required.");
  ...
  app.delete("/api/me", async (request, response) => {
    ...
    if (!authenticated || !store) return;
    try {
      if (projectStore) await projectStore.deleteTenant(authenticated.principal.tenantId);
  ...
  app.post("/api/billing/checkout", async (request, response) => {
    ...
    if (!billing) return response.status(503).json({ ok: false, error: "Checkout is not configured on this instance." });
    try {
  ...
  app.post("/api/billing/confirm", async (request, response) => {
    ...
    if (!billing) return response.status(503).json({ ok: false, error: "Checkout is not configured on this instance." });
    try {
      const sessionId = (request.body as Record<string, unknown>).sessionId;
      if (typeof sessionId !== "string") throw new Error("Checkout session id is required.");
  ...
  app.post("/api/billing/portal", async (request, response) => {
    const authenticated = ownerFor(request, response);
    if (!authenticated || !store) return;
    if (!billing) return response.status(503).json({ ok: false, error: "Billing portal is not configured on this instance." });
    const state = store.billingState(authenticated.principal.tenantId);
    if (!state.customerId) return response.status(409).json({ ok: false, error: "This workspace does not have a confirmed billing customer." });
    try {
  ...
  app.get("/api/billing/invoices", async (request, response) => {
    const authenticated = ownerFor(request, response);
    if (!authenticated || !store) return;
    if (!billing) return response.status(503).json({ ok: false, error: "Invoices are not configured on this instance." });
    const state = store.billingState(authenticated.principal.tenantId);
    if (!state.customerId) return response.json({ ok: true, invoices: [] });
    try {
  ...
  app.post("/api/telemetry", (request, response) => {
    if (!store || !config.telemetryEnabled) return response.status(204).end();
    const authenticated = principalFor(request, response);
    if (!authenticated) return;
    try {
      const { eventName, quantity } = request.body as Record<string, unknown>;
      if (typeof eventName !== "string") throw new Error("Telemetry event name is required.");
  ...
  app.post("/api/projects", async (request, response) => {
    ...
    if (!projectStore) return unavailable(response);
    try {
      const { name, description, build: value } = request.body as Record<string, unknown>;
  ...
  app.get("/api/projects/:projectId", async (request, response) => {
    ...
    try {
      ...
      response.json({
        ok: true,
        project: await withProjectRead(authenticated.principal, () => (
          projectStore.getProjectPage(authenticated.principal.tenantId, request.params.projectId, offset, limit)
        )),
      });
  ...
  app.post("/api/projects/:projectId/versions", async (request, response) => {
    ...
    if (!projectStore) return unavailable(response);
    try {
      const { build: value, reason, expectedHeadVersionId } = request.body as Record<string, unknown>;
      const build = canonicalBuild(value, authenticated.principal);
      if (reason !== undefined && reason !== "manual" && reason !== "autosave" && reason !== "region_revision") throw new Error("Unsupported project version reason.");
      const version = await projectStore.saveVersion({
        tenantId: authenticated.principal.tenantId,
        projectId: request.params.projectId,
        payload: { build, contract: build.contract, certificate: build.certificate },
        reason: reason as "manual" | "autosave" | "region_revision" | undefined,
        createdBy: authenticated.principal.userId,
  ...
  app.get("/api/projects/:projectId/diff", async (request, response) => {
    ...
    try {
      ...
      if (typeof before !== "string" || typeof after !== "string") throw new Error("Both before and after version ids are required.");
      ...
      const diff = await withProjectRead(authenticated.principal, () => (
        projectStore.compareVersionsPage(authenticated.principal.tenantId, request.params.projectId, before, after, offset, limit)
      ));
      const { materialDeltas: _materialDeltas, ...summary } = diff;
      const materialItems: Array<ReturnType<typeof publicMaterialState> & { delta: number }> = [];
      let materialDeltaCount = 0;
      for (const state in diff.materialDeltas) {
        if (!Object.hasOwn(diff.materialDeltas, state)) continue;
  ...
  app.post("/api/projects/:projectId/restore", async (request, response) => {
    ...
    if (!projectStore) return unavailable(response);
    try {
      const { versionId, expectedHeadVersionId } = request.body as Record<string, unknown>;
  ...
  app.post("/api/projects/:projectId/review-links", async (request, response) => {
    const authenticated = await writerFor(request, response);
    if (!authenticated) return;
    if (!projectStore || !reviewService) return unavailable(response);
    try {
      const { versionId, expiresInSeconds } = request.body as Record<string, unknown>;
      if (typeof versionId !== "string") throw new Error("Project version id is required.");
  ...
  app.delete("/api/review-links/:linkId", async (request, response) => {
    const authenticated = await writerFor(request, response);
    if (!authenticated) return;
    if (!reviewService) return unavailable(response);
    try {
  ...
  app.get("/api/review/:token/placements", async (request, response) => {
    ...
    try {
      ...
      if (!Number.isSafeInteger(offset) || offset < 0 || !Number.isSafeInteger(limit) || limit < 1 || limit > 1000) throw new Error("Review placement page must use offset >= 0 and limit between 1 and 1000.");
      const review = await reviewService.resolve(request.params.token);
  ...
  app.post("/api/review/:token/decision", async (request, response) => {
    if (!reviewService) return unavailable(response);
    try {
      const { actorId, decision, comment } = request.body as Record<string, unknown>;
      if (typeof actorId !== "string" || (decision !== "approved" && decision !== "changes_requested")) throw new Error("Reviewer name and a supported decision are required.");
  ...
  app.get("/api/operator/metrics", (request, response) => {
    if (!store || !config.operatorToken) return unavailable(response);
    const token = request.headers.authorization?.startsWith("Bearer ") ? request.headers.authorization.slice(7).trim() : "";
    if (!secureTokenEqual(token, config.operatorToken)) return response.status(401).json({ ok: false, error: "Operator authorization is required." });
    ...
    `).get(new Date().toISOString()) as { tenants: number; users: number; active_sessions: number; pending_refunds: number };
