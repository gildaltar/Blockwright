...
import type { CredentialResolver } from "./model-providers.js";
import { ManagedTaskError, sanitizeDiagnosticText } from "./task-contract.js";

export type CredentialReference = `wincred:blockwright/${string}`;
...
export interface CredentialStore extends CredentialResolver {
  set(reference: CredentialReference, secret: string, signal?: AbortSignal): Promise<CredentialStatus>;
  read(reference: CredentialReference, signal?: AbortSignal): Promise<string | undefined>;
  testRead(reference: CredentialReference, signal?: AbortSignal): Promise<{ reference: CredentialReference; readable: boolean; availability: CredentialStatus["availability"] }>;
  status(reference: CredentialReference, signal?: AbortSignal): Promise<CredentialStatus>;
  list(signal?: AbortSignal): Promise<CredentialDescriptor[]>;
  remove(reference: CredentialReference, signal?: AbortSignal): Promise<{ reference: CredentialReference; removed: boolean }>;
}

export class CredentialStoreError extends ManagedTaskError {
  constructor(input: { code: string; message: string; likelyCause: string; recommendedAction: string; retrySafe?: boolean; retryReason?: string }) {
    super({
      ...input,
...
function validateReference(reference: string): CredentialReference {
  ...
  return reference as CredentialReference;
}

function targetFor(reference: CredentialReference) {
  return `${CREDENTIAL_TARGET_PREFIX}${reference.slice("wincred:blockwright/".length)}`;
}

function referenceFor(target: string): CredentialReference | undefined {
  if (!target.startsWith(CREDENTIAL_TARGET_PREFIX)) return undefined;
  try {
...
export class WindowsCredentialManagerStore implements CredentialStore {
  constructor(private readonly runner: PowerShellRunner = new BoundedPowerShellRunner("win32"), private readonly timeoutMs = 10_000) {
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 60_000) throw new Error("Credential operation timeout must be an integer from 100 through 60000 milliseconds.");
  }

  private async operation<Result>(payload: Record<string, unknown>, signal?: AbortSignal): Promise<Result> {
    ...
    try {
      ...
      const result = JSON.parse(output) as Result & { ok?: unknown };
      if (!result || typeof result !== "object" || result.ok !== true) throw new Error("Credential bridge returned an invalid response.");
      return result;
    } catch (error) {
  ...
  async set(reference: CredentialReference, secret: string, signal?: AbortSignal): Promise<CredentialStatus> {
    const validReference = validateReference(reference);
    if (!secret || secret.includes("\0")) throw new CredentialStoreError({
    ...
    if (encodedLength > 2560) throw new CredentialStoreError({
  ...
  async read(reference: CredentialReference, signal?: AbortSignal): Promise<string | undefined> {
    const validReference = validateReference(reference);
    const result = await this.operation<{ ok: true; exists: boolean; secretUtf8Base64?: unknown }>({ operation: "read", target: targetFor(validReference) }, signal);
    if (!result.exists) return undefined;
    if (typeof result.secretUtf8Base64 !== "string" || !/^[A-Za-z0-9+/]*={0,2}$/.test(result.secretUtf8Base64)) throw new CredentialStoreError({
  ...
  async status(reference: CredentialReference, signal?: AbortSignal): Promise<CredentialStatus> {
    const validReference = validateReference(reference);
    const result = await this.operation<{ ok: true; exists: boolean }>({ operation: "status", target: targetFor(validReference) }, signal);
    return { reference: validReference, availability: result.exists ? "available" : "missing", exists: result.exists };
  }

  async testRead(reference: CredentialReference, signal?: AbortSignal) {
  ...
  async list(signal?: AbortSignal): Promise<CredentialDescriptor[]> {
    const result = await this.operation<{ ok: true; targets: unknown }>({ operation: "list", prefix: CREDENTIAL_TARGET_PREFIX }, signal);
    if (!Array.isArray(result.targets)) throw new CredentialStoreError({
      code: "CREDENTIAL_STORE_INVALID_RESPONSE",
    ...
    return [...new Set(result.targets.flatMap((target) => typeof target === "string" ? [referenceFor(target)] : []).filter((reference): reference is CredentialReference => Boolean(reference)))]
      .sort()
      .map((reference) => ({ reference }));
  }

  async remove(reference: CredentialReference, signal?: AbortSignal) {
    const validReference = validateReference(reference);
    const result = await this.operation<{ ok: true; removed: boolean }>({ operation: "remove", target: targetFor(validReference) }, signal);
...
export class UnsupportedCredentialStore implements CredentialStore {
  ...
  async set(): Promise<CredentialStatus> { return this.unsupported(); }
  async read(): Promise<string | undefined> { return this.unsupported(); }
  async resolve(): Promise<string | undefined> { return this.unsupported(); }
  async testRead(reference: CredentialReference) { return { reference, readable: false, availability: "unsupported" as const }; }
  async status(reference: CredentialReference): Promise<CredentialStatus> { return { reference: validateReference(reference), availability: "unsupported", exists: false }; }
  async list(): Promise<CredentialDescriptor[]> { return this.unsupported(); }
  async remove(): Promise<{ reference: CredentialReference; removed: boolean }> { return this.unsupported(); }
}
...
export function createCredentialStore(options: {
  ...
  if ((options.platform ?? process.platform) !== "win32") return new UnsupportedCredentialStore();
  return new WindowsCredentialManagerStore(options.powerShellRunner ?? new BoundedPowerShellRunner("win32"), options.timeoutMs);
}

export function credentialErrorSummary(error: unknown) {
