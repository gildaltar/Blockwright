...
export type MaterialList = {
  ...
  lines: MaterialLine[];
  baseBlockTotals: { baseBlock: string; count: number; estimatedStacks: number; estimatedShulkerBoxes: number }[];
  planningAid: {
    estimatedStacks: number;
    estimatedShulkerBoxes: number;
    assumptions: string[];
...
export type DeliveryFormat = "schem" | "litematic";
...
export type HashBoundDocument = { buildHash: string };
...
export type DeliveryBundleInput = {
  build: BuildRecord;
  format: DeliveryFormat;
...
function estimateContainers(count: number) {
  const estimatedStacks = Math.ceil(count / 64);
  return { estimatedStacks, estimatedShulkerBoxes: Math.ceil(estimatedStacks / 27) };
}

export function createMaterialList(build: BuildRecord, options: { includeAir?: boolean } = {}): MaterialList {
  const exactCounts = new Map<string, { baseBlock: string; count: number }>();
  const baseCounts = new Map<string, number>();
  for (const placement of build.placements) {
    if (!options.includeAir && placement.block === "minecraft:air") continue;
...
function checksum(content: string | Uint8Array) {
  return createHash("sha256").update(content).digest("hex");
}

function assertHashBound(document: HashBoundDocument, expected: string, label: string) {
  if (document.buildHash !== expected) throw new Error(`${label} is bound to a different build hash.`);
}
...
function defaultCompatibility(build: BuildRecord, format: DeliveryFormat): DeliveryCompatibility {
  if (format === "litematic") {
    return {
      format,
...
function validateCompatibility(build: BuildRecord, format: DeliveryFormat, supplied: DeliveryCompatibility | undefined) {
  ...
  if (!compatibility.notes.length || compatibility.notes.some((note) => !note.trim())) throw new Error("Compatibility metadata requires at least one non-empty limitation or verification note.");
  return compatibility;
}

export async function createDeliveryBundle(input: DeliveryBundleInput) {
  const { build, format } = input;
  if (build.input.edition !== "java") throw new Error("Professional Java delivery bundles require a Java Edition build.");
  assertHashBound(input.contract, build.hash, "Build contract");
  assertHashBound(input.certificate, build.hash, "Review certificate");
  if (build.contract.buildHash !== build.hash || build.certificate.buildHash !== build.hash) throw new Error("Build audit artifacts are stale for the canonical build hash.");
  ...
  if (input.reviewApproval && input.reviewApproval.buildHash !== build.hash) throw new Error("Review approval is bound to a different build hash.");
  if (input.reviewApproval && (
    input.reviewApproval.decision !== "approved"
  ...
  if (!Number.isFinite(Date.parse(generatedAt))) throw new Error("Delivery generation time must be an ISO-compatible timestamp.");
  const origin = input.origin ?? build.bounds.min;
  const rotation = input.rotation ?? 0;
  if (![0, 90, 180, 270].includes(rotation)) throw new Error("Delivery rotation must be 0, 90, 180, or 270 degrees.");
  if (![origin.x, origin.y, origin.z].every(Number.isSafeInteger)) throw new Error("Delivery origin must use safe integer coordinates.");
