...
export type ExportFormat = "json" | "csv" | "java_mcfunction" | "bedrock_mcfunction" | "mcpack" | "blueprint" | "schem" | "bundle";
...
function renderMcfunction(build: BuildRecord, edition: "java" | "bedrock") {
...
export function chunkMcfunction(build: BuildRecord, edition: "java" | "bedrock", chunkSize = 8000) {
  if (!Number.isSafeInteger(chunkSize) || chunkSize < 1) throw new Error("Command chunk size must be a positive whole number.");
  if (edition === "bedrock" && chunkSize > BEDROCK_FUNCTION_COMMAND_LIMIT) throw new Error(`Bedrock command chunks cannot exceed ${BEDROCK_FUNCTION_COMMAND_LIMIT.toLocaleString()} commands.`);
...
export function toBlueprint(build: BuildRecord) {
  ...
  for (let y = minY; y <= maxY; y += 1) {
    ...
    for (let z = build.bounds.min.z; z <= build.bounds.max.z; z += 1) {
      const row: string[] = [];
      for (let x = build.bounds.min.x; x <= build.bounds.max.x; x += 1) row.push(layer.has(`${x},${z}`) ? symbols.get(layer.get(`${x},${z}`)!.block)! : ".");
      lines.push(row.join(" "));
    }
  }
  return lines.join("\n");
...
export async function createBundle(build: BuildRecord) {
  ...
  if (bedrockPack) zip.file(bedrockPack.fileName, bedrockPack.bytes);
