...
export type BedrockPackInventory = {
  schemaVersion: 1;
  blockwrightVersion: string;
  buildId: string;
  buildHash: string;
  ...
  estimatedMaximumBlockChangesPerTick: number;
  parts: number;
  tickingAreas: number;
...
function tickingAreas(build: BuildRecord, namespace: string) {
  const chunkMinX = Math.floor(build.bounds.min.x / 16); const chunkMaxX = Math.floor(build.bounds.max.x / 16);
  const chunkMinZ = Math.floor(build.bounds.min.z / 16); const chunkMaxZ = Math.floor(build.bounds.max.z / 16);
  const areas: Array<{ name: string; minX: number; minZ: number; maxX: number; maxZ: number }> = [];
  for (let chunkZ = chunkMinZ; chunkZ <= chunkMaxZ; chunkZ += 8) for (let chunkX = chunkMinX; chunkX <= chunkMaxX; chunkX += 8) {
  ...
  if (areas.length > 10) throw new Error("BEDROCK_MCPACK_TICKINGAREA_LIMIT_EXCEEDED: split this build into at most ten 8x8-chunk mobile phases.");
  return areas;
}

export async function createBedrockMcpack(build: BuildRecord, commandsPerTick = 32) {
  if (build.input.edition !== "bedrock") throw new Error("BEDROCK_MCPACK_REQUIRES_BEDROCK_BUILD");
  ...
  const chunks: string[][] = [];
  for (let index = 0; index < commands.length; index += boundedCommandsPerTick) chunks.push(commands.slice(index, index + boundedCommandsPerTick));
  const areas = tickingAreas(build, namespace);
  const inventory: BedrockPackInventory = {
    schemaVersion: 1,
