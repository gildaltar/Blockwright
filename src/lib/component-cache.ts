export type ComponentCacheOptions = {
  maxEntries?: number;
  maxWeight?: number;
};
...
export class ComponentCache<Value> {
  ...
  set(key: string, value: Value, weight = 1) {
    ...
    if (resolvedWeight > this.maxWeight) return false;
...
function positiveLimit(value: number | undefined, fallback: number, label: string) {
  ...
  if (!Number.isSafeInteger(resolved) || resolved < 1) throw new Error(`COMPONENT_CACHE_LIMIT_INVALID: ${label} must be a positive safe integer.`);
  return resolved;
}
