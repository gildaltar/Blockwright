import type {
  ...
  DesignPrimitive,
  DesignProfile,
  DesignSurfaceDirection,
  Vec3,
} from "./types.js";

export type ProceduralGeometryPoint = {
  point: Vec3;
  surfaceDirections: DesignSurfaceDirection[];
};

export type ProceduralGeometryResult = {
  points: ProceduralGeometryPoint[];
  bounds: { min: Vec3; max: Vec3 };
  attempts: number;
  collisions: number;
};

type PrimitiveCollector = {
  add: (point: Vec3) => void;
};

export function compileProceduralGeometry(
  ...
  let attempts = 0;
  let collisions = 0;
  const collect = (target: Map<string, Vec3>): PrimitiveCollector => ({
    add(point) {
      assertIntegerPoint(point, primitive.type);
      attempts += 1;
      if (attempts > maximumAttempts) throw new Error(`DESIGN_OPERATION_LIMIT_EXCEEDED: procedural geometry exceeded ${maximumAttempts.toLocaleString()} coordinate candidates.`);
      const key = coordinateKey(point);
      if (target.has(key)) collisions += 1;
  ...
  for (const mask of options.masks ?? []) {
    const maskPoints = new Map<string, Vec3>();
  ...
  const translated = new Map<string, Vec3>();
  for (const point of selected) {
    const moved = add(point, offset);
    const key = coordinateKey(moved);
    if (translated.has(key)) collisions += 1;
...
export function proceduralPrimitiveBounds(primitive: DesignPrimitive) {
  return primitiveBounds(primitive);
}

export function transformProceduralPrimitive(
  primitive: DesignPrimitive,
  translation: Vec3,
  mirror?: { axis: "x" | "z"; coordinate: number },
): DesignPrimitive {
  const point = (value: Vec3) => reflect(add(value, translation), mirror);
  const vector = (value: Vec3) => mirror?.axis === "x" ? { ...value, x: -value.x } : mirror?.axis === "z" ? { ...value, z: -value.z } : { ...value };
  const box = (min: Vec3, max: Vec3) => normalizedBox(point(min), point(max));
  if (primitive.type === "line") return { ...primitive, from: point(primitive.from), to: point(primitive.to) };
  ...
  if (primitive.type === "circle" || primitive.type === "ellipse" || primitive.type === "sphere") return { ...primitive, center: point(primitive.center) };
  if (primitive.type === "cone") return { ...primitive, baseCenter: point(primitive.baseCenter) };
  if (primitive.type === "polygon") return { ...primitive, points: primitive.points.map(point) };
  if (primitive.type === "extrusion") return {
    ...primitive,
    origin: point(primitive.origin),
    profile: transformProfile(primitive.profile, mirror),
    offset: vector(primitive.offset),
  };
  if (primitive.type === "profile_extrusion") return { ...primitive, profile: transformProfile(primitive.profile, mirror), path: primitive.path.map(point) };
  const transformed = box(primitive.min, primitive.max);
  return {
    ...primitive,
    ...transformed,
...
function generatePrimitive(primitive: DesignPrimitive, collector: PrimitiveCollector) {
  if (primitive.type === "line") {
    for (const point of linePoints(primitive.from, primitive.to)) addThickPoint(point, primitive.thickness ?? 1, collector);
    return;
  }
  if (primitive.type === "plane") {
    ...
    if (!flatAxes.length) throw new Error("DESIGN_PRIMITIVE_INVALID: plane must be flat on at least one axis.");
    ...
    forEachBox(min, max, (point) => {
      ...
      if (filled || boundary) collector.add(point);
    });
    return;
  }
  ...
  if (primitive.type === "sphere") {
    ...
    for (let dx = -radius; dx <= radius; dx += 1) for (let dy = -radius; dy <= radius; dy += 1) for (let dz = -radius; dz <= radius; dz += 1) {
      ...
      if (distance > radius + 0.25 || (primitive.hollow && distance < Math.max(0, radius - thickness))) continue;
  ...
  if (primitive.type === "cone") {
    ...
    for (let layer = 0; layer < height; layer += 1) {
      ...
      for (let dx = -Math.ceil(layerRadius); dx <= Math.ceil(layerRadius); dx += 1) for (let dz = -Math.ceil(layerRadius); dz <= Math.ceil(layerRadius); dz += 1) {
        const distance = Math.hypot(dx, dz);
        if (distance > layerRadius + 0.25 || (primitive.hollow && distance < Math.max(0, layerRadius - thickness) && layer > 0)) continue;
  ...
  if (primitive.type === "pyramid") {
    ...
    for (let layer = 0; layer < height; layer += 1) {
      ...
      forEachBox(layerMin, layerMax, (point) => {
        ...
        if (!primitive.hollow || edge < thickness || layer === 0 || layer === height - 1) collector.add(point);
      });
    }
    return;
  }
  ...
  if (primitive.type === "extrusion") {
    ...
    for (const translation of linePoints({ x: 0, y: 0, z: 0 }, primitive.offset)) {
      for (const offset of profile) collector.add(add(primitive.origin, add(offset, translation)));
    }
    return;
  }
  if (primitive.type === "profile_extrusion") {
    if (primitive.path.length < 2) throw new Error("DESIGN_PRIMITIVE_INVALID: profile_extrusion needs at least two path points.");
    const profile = profilePoints(primitive.profile);
    for (const pathPoint of polylinePoints(primitive.path)) for (const offset of profile) collector.add(add(pathPoint, offset));
    return;
  }
  if (primitive.type === "roof_plane") {
...
function rasterEllipse(center: Vec3, axis: "x" | "y" | "z", rawRadiusU: number, rawRadiusV: number, filled: boolean, rawThickness: number, collector: PrimitiveCollector) {
  ...
  for (let u = -radiusU; u <= radiusU; u += 1) for (let v = -radiusV; v <= radiusV; v += 1) {
    ...
    if (outer > 1.08 || (!filled && radiusU > thickness && radiusV > thickness && inner < 1)) continue;
    collector.add(fromPlane(center, axis, u, v));
  }
}

function generateWorldPolygon(points: Vec3[], filled: boolean, collector: PrimitiveCollector) {
  if (points.length < 3) throw new Error("DESIGN_PRIMITIVE_INVALID: polygon needs at least three points.");
  points.forEach((point) => assertIntegerPoint(point, "polygon point"));
  const normal = (["y", "x", "z"] as const).find((axis) => points.every((point) => point[axis] === points[0][axis]));
  if (!normal) throw new Error("DESIGN_PRIMITIVE_UNSUPPORTED: polygon points must lie on one axis-aligned plane.");
  ...
  for (const point of raster) collector.add(worldFrom2d(normal, constant, point.u, point.v));
}

function generateRoundedRectangle(
  primitive: Extract<DesignPrimitive, { type: "rounded_rectangle" }> | Extract<DesignPrimitive, { type: "rounded_square" }>,
  collector: PrimitiveCollector,
) {
  const { min, max } = normalizedBox(primitive.min, primitive.max);
  const normal = axes.find((axis) => min[axis] === max[axis]);
  if (!normal) throw new Error(`DESIGN_PRIMITIVE_INVALID: ${primitive.type} must lie on an axis-aligned plane.`);
  const [uAxis, vAxis] = planeAxes(normal);
  const uMin = min[uAxis]; const uMax = max[uAxis]; const vMin = min[vAxis]; const vMax = max[vAxis];
  if (primitive.type === "rounded_square" && uMax - uMin !== vMax - vMin) throw new Error("DESIGN_PRIMITIVE_INVALID: rounded_square requires equal side spans.");
  ...
  const insideRounded = (u: number, v: number) => {
  ...
  for (let u = uMin; u <= uMax; u += 1) for (let v = vMin; v <= vMax; v += 1) if (insideRounded(u, v)) all.add(`${u},${v}`);
  const thickness = positiveThickness(primitive.thickness);
  for (let u = uMin; u <= uMax; u += 1) for (let v = vMin; v <= vMax; v += 1) {
    if (!all.has(`${u},${v}`)) continue;
    const boundary = Array.from({ length: thickness }, (_, distance) => distance + 1).some((distance) => (
      !all.has(`${u - distance},${v}`) || !all.has(`${u + distance},${v}`) || !all.has(`${u},${v - distance}`) || !all.has(`${u},${v + distance}`)
    ));
    if ((primitive.filled ?? false) || boundary) collector.add(worldFrom2d(normal, min[normal], u, v));
  }
}

function generateRoofPlane(primitive: Extract<DesignPrimitive, { type: "roof_plane" }>, collector: PrimitiveCollector) {
  const { min, max } = normalizedBox(primitive.min, primitive.max);
  const thickness = positiveThickness(primitive.thickness);
  ...
  for (let x = min.x; x <= max.x; x += 1) for (let z = min.z; z <= max.z; z += 1) {
    ...
    if ((primitive.highSide ?? "max") === "min") ratio = 1 - ratio;
    const top = Math.round(min.y + ratio * (max.y - min.y));
    for (let layer = 0; layer < thickness && top - layer >= min.y; layer += 1) collector.add({ x, y: top - layer, z });
  }
}

function generateRoofRidge(primitive: Extract<DesignPrimitive, { type: "roof_ridge" }>, collector: PrimitiveCollector) {
  const { min, max } = normalizedBox(primitive.min, primitive.max);
...
function generateTerrainSurface(primitive: Extract<DesignPrimitive, { type: "terrain_surface" }>, collector: PrimitiveCollector) {
  ...
  for (let x = min.x; x <= max.x; x += 1) for (let z = min.z; z <= max.z; z += 1) {
    ...
    for (let y = bottom; y <= height; y += 1) collector.add({ x, y, z });
  }
}

function profilePoints(profile: DesignProfile) {
  if (profile.points.length < 3) throw new Error("DESIGN_PRIMITIVE_INVALID: extrusion profiles need at least three points.");
  for (const point of profile.points) if (!Number.isSafeInteger(point.u) || !Number.isSafeInteger(point.v)) throw new Error("DESIGN_COORDINATE_INVALID: profile points must be integers.");
...
function polygon2dPoints(points: Array<{ u: number; v: number }>, filled: boolean) {
  const edge = new Map<string, { u: number; v: number }>();
  for (let index = 0; index < points.length; index += 1) for (const point of line2d(points[index], points[(index + 1) % points.length])) edge.set(`${point.u},${point.v}`, point);
  if (!filled) return [...edge.values()];
  const minU = Math.min(...points.map(({ u }) => u)); const maxU = Math.max(...points.map(({ u }) => u));
  const minV = Math.min(...points.map(({ v }) => v)); const maxV = Math.max(...points.map(({ v }) => v));
  const result = new Map(edge);
  for (let u = minU; u <= maxU; u += 1) for (let v = minV; v <= maxV; v += 1) {
    if (pointInPolygon(u, v, points)) result.set(`${u},${v}`, { u, v });
  }
...
function pointInPolygon(u: number, v: number, polygon: Array<{ u: number; v: number }>) {
  ...
  for (let left = 0, right = polygon.length - 1; left < polygon.length; right = left++) {
    const a = polygon[left]; const b = polygon[right];
    if ((a.v > v) !== (b.v > v) && u < ((b.u - a.u) * (v - a.v)) / (b.v - a.v) + a.u) insidePolygon = !insidePolygon;
  }
  return insidePolygon;
}

function line2d(from: { u: number; v: number }, to: { u: number; v: number }) {
  const points: Array<{ u: number; v: number }> = [];
  const du = to.u - from.u; const dv = to.v - from.v;
  const steps = Math.max(Math.abs(du), Math.abs(dv), 1);
  for (let step = 0; step <= steps; step += 1) points.push({ u: Math.round(from.u + du * step / steps), v: Math.round(from.v + dv * step / steps) });
  return points;
}

function linePoints(from: Vec3, to: Vec3) {
  assertIntegerPoint(from, "line.from"); assertIntegerPoint(to, "line.to");
...
function polylinePoints(points: Vec3[]) {
  const result = new Map<string, Vec3>();
  for (let index = 0; index < points.length - 1; index += 1) for (const point of linePoints(points[index], points[index + 1])) result.set(coordinateKey(point), point);
  return [...result.values()];
}

function addThickPoint(point: Vec3, thickness: number, collector: PrimitiveCollector) {
  const size = positiveThickness(thickness);
  const low = -Math.floor((size - 1) / 2); const high = Math.ceil((size - 1) / 2);
  for (let dx = low; dx <= high; dx += 1) for (let dy = low; dy <= high; dy += 1) for (let dz = low; dz <= high; dz += 1) collector.add({ x: point.x + dx, y: point.y + dy, z: point.z + dz });
}

function fromPlane(center: Vec3, axis: "x" | "y" | "z", u: number, v: number): Vec3 {
...
function planeAxes(normal: "x" | "y" | "z"): ["x" | "y" | "z", "x" | "y" | "z"] {
  return normal === "x" ? ["y", "z"] : normal === "y" ? ["x", "z"] : ["x", "y"];
}

function worldFrom2d(normal: "x" | "y" | "z", constant: number, u: number, v: number): Vec3 {
  return normal === "x" ? { x: constant, y: u, z: v } : normal === "y" ? { x: u, y: constant, z: v } : { x: u, y: v, z: constant };
}

function primitiveBounds(primitive: DesignPrimitive): { min: Vec3; max: Vec3 } {
  if (primitive.type === "line") {
  ...
  if (primitive.type === "plane" || primitive.type === "pyramid" || primitive.type === "rounded_rectangle" || primitive.type === "rounded_square"
    || primitive.type === "roof_plane" || primitive.type === "roof_ridge" || primitive.type === "terrain_surface") return normalizedBox(primitive.min, primitive.max);
  if (primitive.type === "circle" || primitive.type === "ellipse") {
  ...
  if (primitive.type === "sphere") return { min: { x: primitive.center.x - primitive.radius, y: primitive.center.y - primitive.radius, z: primitive.center.z - primitive.radius }, max: { x: primitive.center.x + primitive.radius, y: primitive.center.y + primitive.radius, z: primitive.center.z + primitive.radius } };
  if (primitive.type === "cone") {
  ...
  if (primitive.type === "polygon") return boundsOf(primitive.points);
  if (primitive.type === "extrusion") {
...
function profileVector(plane: DesignProfile["plane"], u: number, v: number): Vec3 {
  return plane === "xy" ? { x: u, y: v, z: 0 } : plane === "xz" ? { x: u, y: 0, z: v } : { x: 0, y: u, z: v };
}

function transformProfile(profile: DesignProfile, mirror?: { axis: "x" | "z"; coordinate: number }): DesignProfile {
  if (!mirror) return structuredClone(profile);
  return {
    ...profile,
    points: profile.points.map(({ u, v }) => {
      if ((profile.plane === "xy" || profile.plane === "xz") && mirror.axis === "x") return { u: -u, v };
      if (profile.plane === "xz" && mirror.axis === "z") return { u, v: -v };
      if (profile.plane === "yz" && mirror.axis === "z") return { u, v: -v };
      return { u, v };
    }),
  };
}

function exposedDirections(point: Vec3, occupied: Set<string>): DesignSurfaceDirection[] {
  const directions: Array<[DesignSurfaceDirection, Vec3]> = [
...
function valueNoise2d(x: number, z: number, scale: number, seed: string) {
  const sx = x / scale; const sz = z / scale; const x0 = Math.floor(sx); const z0 = Math.floor(sz);
  const tx = smooth(sx - x0); const tz = smooth(sz - z0);
  const sample = (dx: number, dz: number) => hashUnit(`${seed}:${x0 + dx},${z0 + dz}`);
  return lerp(lerp(sample(0, 0), sample(1, 0), tx), lerp(sample(0, 1), sample(1, 1), tx), tz);
}

function boundsOf(points: Vec3[]) {
  if (!points.length) throw new Error("DESIGN_PRIMITIVE_INVALID: geometry must contain at least one point.");
  const min = { ...points[0] }; const max = { ...points[0] };
  for (const point of points.slice(1)) for (const axis of axes) {
...
function normalizedBox(left: Vec3, right: Vec3) {
...
function forEachBox(min: Vec3, max: Vec3, visit: (point: Vec3) => void) {
  for (let x = min.x; x <= max.x; x += 1) for (let y = min.y; y <= max.y; y += 1) for (let z = min.z; z <= max.z; z += 1) visit({ x, y, z });
}

function linearlyContained(value: number, min: number, max: number) {
  return value >= min && value <= max;
}

function inside(point: Vec3, bounds: { min: Vec3; max: Vec3 }) {
...
function reflect(point: Vec3, mirror?: { axis: "x" | "z"; coordinate: number }) {
...
function coordinateKey({ x, y, z }: Vec3) {
  return `${x},${y},${z}`;
}
...
function positiveRadius(value: number, label: string) {
  if (!Number.isFinite(value) || value <= 0) throw new Error(`DESIGN_PRIMITIVE_INVALID: ${label} must be positive.`);
  return Math.max(1, Math.round(value));
}

function positiveInteger(value: number, label: string) {
  if (!Number.isSafeInteger(value) || value < 1) throw new Error(`DESIGN_PRIMITIVE_INVALID: ${label} must be a positive integer.`);
  return value;
}

function positiveThickness(value: number | undefined) {
  if (value !== undefined && (!Number.isFinite(value) || value <= 0)) throw new Error("DESIGN_PRIMITIVE_INVALID: thickness must be positive.");
