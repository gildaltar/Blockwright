...
- Defined v0.6.0 as an explicitly unsigned Windows MVP prerelease with exact-tag downloads, SmartScreen/unknown-publisher instructions, checksum and SBOM evidence, manual upgrades, and no claim that it satisfies the signed stable-release or WinGet gates.
- Made the protected signed-release workflow manual-only so an unsigned tag cannot start a credentialed stable-release run; its qualified-tag, signing, provenance, draft, and approval gates remain intact for a future signed release.
